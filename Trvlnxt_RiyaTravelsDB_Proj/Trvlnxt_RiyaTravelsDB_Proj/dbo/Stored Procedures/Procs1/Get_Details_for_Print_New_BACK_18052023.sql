--Get_Details_for_Print_New 'RT20230505105608168', 'OQ7906', '45G5XM', 'USD', NULL, '849902'
CREATE Procedure [dbo].[Get_Details_for_Print_New_BACK_18052023]
...
BEGIN
	...
	WHERE t2.orderId = @NewOrderID 
	AND (t1.isReturn = 0 OR (t1.isReturn=1 and (SELECT count(pkId) FROM tblBookMaster WHERE orderId=@NewOrderID) =1))
	and (t1.totalFare > 0 or (ISNULL(t2.PreviousRiyaPNR,'')!='') or t1.paxType='Infant')
	...
	--GST details
	SELECT RegistrationNumber,	CompanyName,CAddress+','+CState AS CAddress,CState,CContactNo, CEmailID
	FROM tblBookMaster t 
	...
	SELECT 
		...
		,CASE WHEN c.CurrencyCode !=@Currency THEN
					...
					+ ISNULL(tp.ServiceFee,0) + ISNULL(tp.gst,0) + ISNULL(tp.HupAmount,0)  --+ ISNULL(tb.ServiceFee,0) + ISNULL(tb.GST,0) 
				ELSE SUM(CAST((tp.totalFare) * tb.ROE AS decimal(10,2))) --+   ISNULL(tp.MCOAmount,0) +  ISNULL(tp.Markup,0)
					...
					+ ISNULL(tp.ServiceFee,0) + ISNULL(tp.gst,0)  --+ ISNULL(tb.ServiceFee,0) + ISNULL(tb.GST,0) 
			END totalFare
		,(CASE WHEN c.CurrencyCode != @Currency 
