--GetBookingConfirmedDetailsNew'TNH00273665','',21958                                                                                                  
CREATE PROCEDURE GetBookingConfirmedDetailsNew                                                                                                                                                                            
...
--CONVERT(varchar,HB.CheckInDate,6) as CheckInDate,                                                                         
   ...
   ISNULL(HB.ContractComment,'') as ContractComment,                                                                                                                                                              
 ...
 AL.Address as AgentAddress,          
   ...
   ISNULL(HB.Nationalty,'') as AgentNatinality,                        
   HB.PassengerPhone,                                                 
...
------> Confirmation details                                   
 ...
 HB.CurrencyCode as 'Currency',                                                                               
   ...
   ,HB.RiyaAgentID                                                 
   ,HB.OBTCNo                         
   ,isnull(HB.MBPageOBTCNo,'') as MBPageOBTCNo                        
   ,HB.SpecialRemark                                                                                                                   
 ...
 ISNULL(HB.BookingPortal,'TNH') AS BookingPortal,                                              
  ...
  ISNULL(HB.IsPANCardRequired,0) AS IsPANCardRequired,                                  
  ISNULL(BR.Icast,'') AS AgentIcast,                                
  ISNULL(HB.ModifiedCheckInTime,'') as ModifiedCheckInTime,                                
  ISNULL(HB.ModifiedCheckOutTime,'') as ModifiedCheckOutTime,                                
  ISNULL(HB.ServiceTimeModified,0) as ServiceTimeModified,                                
  ISNULL(HB.CorporatePANVerificatioStatus,'') as CorporatePANVerificatioStatus,                           
  ISNULL(HB.HotelAdditionalInformation,'') as HotelAdditionalInformation,                  
  ISNULL(MAGst.RegistrationNumber, '') as AgentGSTNumber,                  
  ISNULL(MAGst.Email, '') as AgentGSTEmail,                  
  ISNULL(MAGst.ContactNo, '') as AgentGSTContactNo,                  
  ISNULL(MAGst.CompanyAddress, '') as AgentGSTCompanyAddress,                  
  ISNULL(MGSTS.GSTState, '') as AgentGSTState,              
  ISNULL(HB.agentCancellationCharges,0) as agentCancellationCharges,            
  ISNULL(HB.HotelInformation,'') as HotelInformation,          
  ISNull(HB.PanCardURL,'') as PanCardURL          
  ----> B2BHotel_Commission                                                                                             
  ...
  ,ISNULL(HB.TotalServiceCharges,0) as TotalServiceCharges     
  ,ISNULL(AL.BalanceFetch,'') as AgentBalanceFetch  
  ,ISNULL(AL.userTypeID,'') as AgentuserTypeID  
  ,ISNULL(AL.Country,'') as AgentCountry  
  ,ISNULL(BR.EntityType,'') as AgentEntityType  
...
Left Join mUser MU1 on HB.MainAgentID = MU1.ID                                                                                                                        
  ...
  Left Join mAgentGSTMapping MAGst on MAGst.AgentID=HB.RiyaAgentID                  
  left join mGstState MGSTS on MGSTS.ID=MAGst.State                  
...
-- passenger details                                                                                                                                                      
  select ISNULL(PAX.MealBasis,'') as MealBasis,ISNULL(ID,0) as PaxId,                                            
  --case when ISDATE(PAX.IssueDate)=1 then PAX.IssueDate else CONVERT(date, PAX.IssueDate, 103) end as IssueDate,                                 
  --case when ISDATE(PAX.Expirydate)=1 then PAX.Expirydate else CONVERT(date, PAX.Expirydate, 103) end as Expirydate,                                            
PAX.* from Hotel_Pax_master as PAX                                                                                                                                             
  INNER JOIN Hotel_BookMaster HB ON PAX.book_fk_id=HB.pkId                                                                                                                                                                                                     
                                                                                       
  where (HB.pkId = @pkid or @pkid=null or (HB.BookingReference=@bookingid and BookingReference!='') or @bookingid=null) and RiyaAgentID=@AgentId  order by IsLeadPax desc                                                                                      
  
                         
--- Room Details                                                                                                                                         
  select ISNULL(Room.RoomMealBasis,'') as RoomMealBasis,ISNULL(Room.RoomDiscription,'') as RoomDiscription,ISNULL(Room.Room_Id,0) as roomid,  Room.* from Hotel_Room_master as Room                                                                            
...
---- cancellation pollicy                                                                    
 ...
 select isnull(HBCP.CPStardDate,GETDATE()) as CPStardDate, isnull(HBCP.CPEndDate,GETDATE()) as CPEndDate, HBCP.* from  [Hotel].HotelCancelBookPolicies as HBCP                                                                                       
  ...
  select isnull(AC.Text,'') as Text, isnull(AC.Description,'') as Description,isnull(AC.Amount,0) as Amount,isnull(AC.Currency,'') as Currency,isnull(AC.RoomNo,0) as RoomNo  from hotel.AdditionalCharges as AC                                               
  ...
  select isnull(AI.RateCode,'') as RateCode,isnull(AI.RateText,'') as RateText from hotel.AdditionalInformation as AI                                                               
  INNER JOIN Hotel_BookMaster HB ON AI.book_fk_id=HB.pkId                                                                 
  where (HB.pkId = @pkid or @pkid=null or (HB.BookingReference=@bookingid and BookingReference!='') or @bookingid=null)  and RiyaAgentID=@AgentId                    
              
   ----- Booking Amenities Details                  
  select isnull(HA.FacilitieName,'') as FacilitieName,isnull(HA.FaciliyieIcon,'') as FaciliyieIcon from HotelBookingAmenities as HA                                                               
  INNER JOIN Hotel_BookMaster HB ON HA.BookingFkid=HB.pkId                                                                 
  where (HB.pkId = @pkid or @pkid=null or (HB.BookingReference=@bookingid and BookingReference!='') or @bookingid=null)  and RiyaAgentID=@AgentId         
      
     ----- Booking Modify Details        
   Select ISNULL(HBMD.book_fk_id,0) as book_fk_id,ISNULL(HBMD.room_fk_id,0) as room_fk_id,ISNULL(HBMD.pax_fk_id,0) as pax_fk_id,ISNULL(HBMD.Title,'')as Title,        
   ISNULL(HBMD.FName,'')as FName,ISNULL(HBMD.LName,'')as LName,ISNULL(HBMD.RoomType,'') as RoomType,ISNULL(HBMD.RoomMealBasis,'') as RoomMealBasis,
   ISNULL(HBMD.IsLeadPax,0) as IsLeadPax from hotel.HotelBookingModifyDetails AS HBMD        
     INNER JOIN Hotel_BookMaster HB ON HBMD.book_fk_id=HB.pkId and  HBMD.IsActive=1                                                              
  where (HB.pkId = @pkid or @pkid=null or (HB.BookingReference=@bookingid and BookingReference!='') or @bookingid=null)  and RiyaAgentID=@AgentId         
END 