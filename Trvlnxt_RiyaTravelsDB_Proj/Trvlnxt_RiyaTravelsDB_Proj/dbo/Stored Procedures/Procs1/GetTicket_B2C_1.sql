create PROCEDURE [dbo].[GetTicket_B2C_1] -- [dbo].[GetTicket] 'RT20231007195547944',null
...
SELECT TOP 1  
			...
			CASE WHEN b.Country = 'IN' THEN CONVERT(VARCHAR(20),CONVERT(decimal(18,0), b.totalFare * b.ROE)) - isnull((b.FlatDiscount + b.IATACommission + b.PLBCommission + b.PromoDiscount),0) ELSE CONVERT(VARCHAR(20),CONVERT(decimal(18,2),b.totalFare * b.ROE)) - isnull((b.FlatDiscount + b.IATACommission + b.PLBCommission + b.PromoDiscount),0) END as totalFare
			...
			FROM tblBookItenary T,tblBookMaster B
				LEFT OUTER JOIN tblBookMaster B1 ON B.orderId = B1.orderId AND B1.returnFlag = 1
				LEFT OUTER JOIN tblBookMaster B11 ON B.orderId = B11.orderId AND B11.returnFlag 
= 0
				
				JOIN tblPassengerBookDetails P ON P.fkBookMaster = B.pkId 
				LEFT OUTER JOIN tblPassengerBookDetails P1 ON P1.fkBookMaster = B1.pkId

				JOIN Paymentmaster Pm ON Pm.order_id = B.orderId 
				LEFT OUTER JOIN  Paymentmaster Pm1 ON Pm1.order_id = B1.orderId
...
JOIN tblBookMaster B ON B.pkId = P.fkBookMaster
JOIN tblBookItenary I ON B.pkId = I.fkBookMaster
...
FROM tblBookItenary I
JOIN tblBookMaster B ON B.pkId = I.fkBookMaster
WHERE I.orderId =
...
select amount from Paymentmaster where order_id=@OrderId
