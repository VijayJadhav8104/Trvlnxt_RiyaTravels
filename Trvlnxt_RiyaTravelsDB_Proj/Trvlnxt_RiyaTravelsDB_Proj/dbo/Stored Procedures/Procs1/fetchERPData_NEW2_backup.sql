create procedure [dbo].[fetchERPData_NEW2_backup] --[fetchERPData_NEW2]'12-FEB-2020','12-FEB-2020',1,1,'in','1',''
...
(CASE  WHEN p.airCode='SG' THEN 'SG'  WHEN p.airCode='G8' THEN 'G8' 
 WHEN p.airCode  in (SELECT AirlineCode FROM TblSTSAirLineMaster) then 'STS' 
 WHEN p.airCode  in (SELECT AirlineCode1 FROM TblSTSAirLineMaster) then 'STS' ELSE '1A' END) AS 'CRS',
 p.fromAirport,p.toAirport,p.equipment,baggage,FlyingHour,ProductType,'PRD100101' AS ProductCode,
...
PaxType, '' FromCity,'' ToCity,p.orderId,isReturn,(CASE  WHEN p.airCode='SG' THEN 'SG'  WHEN p.airCode='G8' THEN 'G8' 
 WHEN p.airCode  in (SELECT AirlineCode FROM TblSTSAirLineMaster) then 'STS' 
 WHEN p.airCode  in (SELECT AirlineCode1 FROM TblSTSAirLineMaster) then 'STS' ELSE '1A' END) AS 'CRS',p.fromAirport,p.toAirport,p.equipment,baggage,FlyingHour,ProductType,'PRD100101' AS ProductCode,
 '' BranchCode,'' [Emp Code],
 b.RegistrationNumber,b.CompanyName,b.CAddress,b.CState,b.CContactNo,b.CEmailID
 ,JobCodeBookingGivenBy, VesselName, ReasonofTravel, TravelRequestNumber, CostCenter, BudgetCode, EmpDimession, SwonNo, TravelerType, Location, Department, Grade, Bookedby, Designation, Chargeability, NameofApprover, ReferenceNo, TR_POName, RankNo, AType, BookingReceivedDate
...
VendorNo,
PaxType, '' FromCity,'' ToCity,airlinePNR,rbd,
CASE WHEN p.VendorName='Sabre' then '1S' ELSE(
(CASE  WHEN p.airCode='SG' THEN 'SG'  WHEN p.airCode='G8' THEN 'G8' WHEN p.airCode='AI' THEN '1A'
 WHEN p.airCode  in (SELECT AirlineCode FROM TblSTSAirLineMaster) then 'STS' 
 WHEN p.airCode  in (SELECT AirlineCode1 FROM TblSTSAirLineMaster) then 'STS' ELSE '1A' END)) end AS 'CRS',p.fromAirport,p.toAirport,p.equipment,baggage,FlyingHour,
ProductType,'PRD100101' AS ProductCode,BranchCode,EmpCode as [Emp Code],
b.RegistrationNumber,b.CompanyName,b.CAddress,b.CState,b.CContactNo,b.CEmailID,
JobCodeBookingGivenBy,VesselName, ReasonofTravel, TravelRequestNumber, CostCenter, BudgetCode, EmpDimession, SwonNo, TravelerType, Location, Department, Grade, Bookedby, Designation, Chargeability, NameofApprover, ReferenceNo, TR_POName, RankNo, AType, BookingReceivedDate
...
when book.OfficeID ='DXBAD3359' THEN '86215290' when book.OfficeID ='NYC1S21E9' THEN '33750021' ELSE '' END) AS [IATA Code],p.EmpCode,VendorName
...
r.LocationCode,r.BranchCode,PMT.payment_mode,a.UserName,p.EmpCode,rbd,VendorName
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[fetchERPData_NEW2_backup] TO [rt_read]
    AS [dbo];
