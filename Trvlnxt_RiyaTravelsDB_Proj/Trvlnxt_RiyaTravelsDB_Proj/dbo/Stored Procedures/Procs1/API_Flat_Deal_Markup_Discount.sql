CREATE proc [dbo].[API_Flat_Deal_Markup_Discount]    
@Travelfrom datetime,    
@Travelto datetime,    
...
WHERE    
 (((@TravelFrom between TravelFrom and TravelTo ) OR TravelFrom is null)   
 AND ((@TravelTo between TravelFrom and TravelTo) OR TravelTo is null))    
 AND ((SaleFrom <=  CONVERT(DATE,GETDATE()) OR SaleFrom IS NULL)   
 AND (SaleTo  >= CONVERT(DATE,GETDATE()) OR SaleTo IS NULL))     
...
(((@TravelFrom between  TravelValidityFrom and TravelValidityTo ) OR TravelValidityFrom is null)   
AND ((@TravelTo between TravelValidityFrom and TravelValidityTo) OR TravelValidityTo is null))    
AND (((CONVERT(DATE,GETDATE()) between  SaleValidityFrom and SaleValidityTo ) OR SaleValidityFrom is null)   
AND ((CONVERT(DATE,GETDATE()) between SaleValidityFrom and SaleValidityTo) OR SaleValidityTo is null))    
