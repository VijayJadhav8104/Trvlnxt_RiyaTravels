Create procedure [dbo].[fetchERPData_NEW21809201] --fetchERPData_NEW2 '06-sep-2021','06-sep-2021',2,3,'in','3',''
...
,pmt.cardtype,pmt.cardnumber
, pax.SupplierPenalty, pax.reScheduleCharge, pax.RescheduleMarkup, book.PreviousRiyaPNR, #temp.PreviousAirlinePNR,
pax.baggagefare as [Extra Baggage Amount],	0 as [Seat Preference Charge],	0 as [Meal Charges],	0 as [Wheel Chair Charges],	0 as [SSR Comb Amount]
...
left join  #temp on pmt.order_id=#temp.orderId	where (book.IsBooked=1 or book.BookingStatus=1) AND book.Country in (select C.CountryCode  from mUserCountryMapping UM
...
,ROE,AgentROE,pmt.cardtype,pmt.cardnumber
...
pax.IATACommission AS IataCommissionC, book.VendorCommissionPercent	as IataCommissionV,pax.serviceCharge as MarkupOnTax
,book.SupplierCode,book.Vendor_No,book.promoCode,book.Country,book.OfficeID as OfficeID,pmt.cardtype,pmt.cardnumber
...
left join tblBookMaster book on pax.fkBookMaster=book.pkId
left join Paymentmaster pmt on book.orderId=pmt.order_id
left join  #temp4 on pmt.order_id=#temp4.orderId	where (book.IsBooked=1 or book.BookingStatus=1) AND book.Country in (select C.CountryCode  from mUserCountryMapping UM
			INNER JOIN mCountry C ON C.ID=UM.CountryId where UserID=@Userid  AND IsActive=1) 
and (convert(date,book.inserteddate) >=convert(date,@frmDate) and convert(date,book.inserteddate) <=convert(date,@toDate)
...
cast((((pax.totalFare )  -pax.serviceCharge) * book.ROE)as decimal(18,2))  as TotalFare,
cast((isnull((CASE WHEN BOOK.B2bFareType=2 or book.B2bFareType=3 THEN  (pax.B2BMarkup/isnull(book.AgentROE,0)) ELSE 0 END)* book.ROE,0))as decimal(18,2)) as MarkupOnTax,
(cast((isnull((CASE WHEN BOOK.B2bFareType=1 THEN  (pax.B2BMarkup/isnull(book.AgentROE,0)) ELSE 0 END)* book.ROE,0))as decimal(18,2)) +PAX.BFC) as MarkupOnFare,
cast((convert(float,[YQ]* book.ROE))as decimal(18,2)) as YQTax,
cast((isnull(pax.[YRTax]* book.ROE,0))as decimal(18,2)) as YrTax,
cast((isnull(pax.[INTax]* book.ROE,0))as decimal(18,2)) as INTax,
cast((isnull(pax.[JNTax]* book.ROE,0) )as decimal(18,2)) as JNTax,
...
when book.OfficeID ='INTDXBA122' THEN 'INTDXBA122'
when book.OfficeID ='FAE0828' THEN 'FAE0828'
when book.OfficeID ='GSADBRIY' THEN 'GSADBRIY'
when book.OfficeID ='3D0J' THEN '14369644'
 ELSE '' END) AS [IATA Code],VendorName,A.userTypeID,book.orderId,book.ROE,book.AgentROE,book.RegistrationNumber,book.CompanyName,book.CAddress,book.CState,book.CContactNo,book.CEmailID
...
,pmt.cardtype,pmt.cardnumber,pax.profession as 'Rankno'
...
INNER JOIN mCountry C ON C.ID=UM.CountryId where UserID=@Userid  AND IsActive=1) 
and (convert(date,isnull(book.inserteddate_old,book.inserteddate)) >=convert(date,@frmDate) and convert(date,isnull(book.inserteddate_old,book.inserteddate)) <=convert(date,@toDate)
or @frmDate is null and @toDate is null ) and A.UserTypeid=@usertype and book.Country=@country 
and AgentID =(CASE @agentid WHEN '' then agentid else @agentid end) and book.totalFare>0 and pax.totalFare>0
...
,pmt.cardtype,pmt.cardnumber,case when CheckboxVoid=1 then 'VOID' ELSE 'RFND' END  as TransactionType,pax.profession as 'Rankno'
...
left join tblOwnerCurrency O on O.OfficeID=book.OfficeID
where book.BookingStatus=4 AND book.Country in (select C.CountryCode  from mUserCountryMapping UM
...
book.orderId,pax.isReturn,(0-(isnull(book.FlatDiscount,0) + isnull(book.PLBCommission,0) + cast(isnull(book.PromoDiscount/BOOK.ROE,0) as decimal(16,2)))) as PlbC, 
pax.IATACommission AS IataCommissionC, book.VendorCommissionPercent	as IataCommissionV,pax.serviceCharge as MarkupOnTax
,book.SupplierCode,book.Vendor_No,book.Country,book.OfficeID as OfficeID,pmt.cardtype,pmt.cardnumber
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[fetchERPData_NEW21809201] TO [rt_read]
    AS [dbo];
