...
--exec [B2BHotel_SearchBookings] @BookingStatus=4 @BookingID='RT2716611'                                   
--,@ExcelFlag='Excel'                                                                                        
--exec [B2BHotel_SearchBookings] @BookingFromDate='01/02/2025',@BookingToDate='11/02/2025'                            
--,@ExcelFlag='Excel'                                                                                   
                                                                                                                                                                         
                                                                                                                                                                                                                                                               
 -- exec [B2BHotel_SearchBookings] @BookingFromDate='2025-06-01',@BookingToDate='2025-06-01',@ExcelFlag='Excel'
    
      
        
CREATE PROCEDURE [dbo].[B2BHotel_SearchBookings]                                           
...
BEGIN                                      
if(@BookingFromDate='' AND @BookingToDate='' and @BookingID IS NULL)                                                                                                                                                                                           
...
set @BookingFromDate='01/03/2025'                                                                                                                                                                                                                              
...
set @BookingToDate=GETDATE()                                                                               
 ...
 begin                                                                                    
  ...
  select distinct  top 400                                                                                   
   ...
   ,HB.orderId as 'OrderID' ,Case When HM.Status = 'Confirmed' Then '<span style="color:blue; ">Hold</span>'+' / '+                                            
...
,CASE                                                                  
    WHEN hb.ServiceTimeModified = 1 THEN                                                                                           
        FORMAT(HB.CheckInDate, 'dd MMM yyyy') + ' ' + SUBSTRING(HB.ModifiedCheckInTime, 1, 8)                                                   
    ELSE                                                                                                         
        FORMAT(HB.CheckInDate,'dd MMM yyyy')+' '+ HB.CheckInTime                                                                                                        
END AS ServiceDate                                                                  
                                                                                                                                                                                                                                 
 ,convert(varchar,CancellationDeadLine,0) as DeadlineDate            
--,FORMAT(CAST(HB.ExpirationDate AS datetime),'dd MMM yyyy hh:mm tt') as DeadlineDate          
...
when B2BPaymentMode=2 then '<span style="color:Black; font-weight:bold">Credit Limit</span>'                                 
 ...
 --  ,HB.LeaderTitle+' '+HB.LeaderFirstName+' '+HB.LeaderLastName  as 'TravellerName'        
   ...
   ,case when HD.book_fk_id=HB.pkId and HD.IsActive=1 and HD.IsLeadPax=1             
   then HD.Title +' '+HD.FName +' '+ HD.LName else HB.LeaderTitle+' '+HB.LeaderFirstName+' '+HB.LeaderLastName end as 'TravellerName'        
...
from Hotel_BookMaster HB  WITH (NOLOCK)                                                                                                                                                                                     
 ...
 -- left join Hotel_Status_History HSH on HB.book_Id=HSH.FKHotelBookingId                                                                                                                                              
  ...
  left join  mBranch MB WITH (NOLOCK) on MB.BranchCode=BR.BranchCode      
   LEFT Join Hotel.HotelBookingModifyDetails HD on HB.pkId=HD.book_fk_id and HD.IsLeadPax=1 and HD.IsActive=1             
...
( ( @Consultant ='') or (HB.MainAgentID IN  (select Data from sample_split(@Consultant,','))) )                                                       
 ...
 and( (@BookingID ='') or (HB.BookingReference IN  (select Data from sample_split(@BookingID,','))) )                                                                                                                                                
  and( (@BookingStatus ='') or (SH.FkStatusId IN  (select Data from sample_split(@BookingStatus,','))) )                                                                                                                     
   ...
   and( (@Country ='') or (coalesce(HB.HotelBookCountryName,HB.CountryName) IN  (select cast(Data as varchar) from sample_split(@Country,','))) ) --commented by sana                                                                                          
...
--and book_Id is not null                                                                                                                                                                                                                          
 ...
 --declare @modifiedName varchar(200);        
 --set @modifiedName=(select (Title+' '+FName+' '+LName)  from hotel.HotelBookingModifyDetails HDS left join Hotel_BookMaster HB on Hb.pkId=HDS.book_fk_id  where book_fk_id=@Id and IsActive=1 and IsLeadPax=1)         
...
END AS 'Agent ID'                                      
  ...
  -- end  as 'User Name'                                                     
   ...
   --   ,case when HD.book_fk_id=HB.pkId and HD.IsActive=1 and HD.IsLeadPax=1             
   --then HD.Title +' '+HD.FName +' '+ HD.LName else HB.LeaderTitle+' '+HB.LeaderFirstName+' '+HB.LeaderLastName end as 'Leader Guest Name'                 
...
,case when HU.UpdatedType='Passenger Details ReConfirmation' then HU.InsertedDate  end as 'Date of Reconfirmation'                                                  
 ...
 ,Isnull(MRecon.FullName ,'-') as 'Recon By'             
 ,FORMAT(HB.ReconDate , 'dd-MM-yyyy HH:mm')  as ReconDate            
...
ON Br.BranchCode = mybranch.BranchCode                   
   ...
   left join Hotel_ROE_Booking_History  Roe WITH (NOLOCK) on Roe.FkBookId=Hb.pkId                                                                              --left join B2BMakepaymentCommission MC on MC.FkBookId=HB.pkId                                  
  ...
  left join mCommon  mcn WITH (NOLOCK) on mcn.ID=AGL.userTypeID                                                                                                 
   ...
   left join mUser MRecon on Hb.ReconBy = MRecon.ID              
...
--and( (@Supplier ='') or (HB.SupplierName IN  (select Data from sample_split(@Supplier,','))) )                                                                                                                                                               
 ...
 and(HB.VoucherNumber = @VoucherID   or @VoucherID='')                                                                                                                      
  ...
  and( (@Country ='') or  (coalesce(HB.HotelBookCountryName,HB.CountryName) IN  (select cast(Data as varchar) from sample_split(@Country,','))))          -- changed column from countrypkid to countryname  by aman                                           
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[B2BHotel_SearchBookings] TO [rt_read]
    AS [dbo];
