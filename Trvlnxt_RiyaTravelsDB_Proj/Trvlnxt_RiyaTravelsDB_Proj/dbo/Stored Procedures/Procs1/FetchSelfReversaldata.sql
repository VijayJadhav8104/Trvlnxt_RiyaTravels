...
create PROCEDURE [dbo].FetchSelfReversaldata  --[FetchRefundAndCancellation] '39RW0O',''
	@RiyaPNR varchar(8),
	@GDSPNR varchar(8)
AS
BEGIN
...
BEGIN
	...
	case when  len(ticketNum)=8 then substring(ticketNum,1,6) else ticketNum end as ticketNum,title,
	P.CancelledDate,p.RefundedDate,B.OfficeID,B.Country,B.ROE,ISNULL(AOI.Currency,'INR') AS Currency,p.paxType,airCode,B.FareType ,
	B.GDSPNR,IsBooked  from tblBookMaster B
	join tblPassengerBookDetails P on B.pkId = P.fkBookMaster
	LEFT JOIN tblAmadeusOfficeID AOI ON AOI.OfficeID=B.OfficeID
	...
	--Fare Details
	 select DISTINCT(B.frmsector +'-'+ B.tosector)  as sector, totalFare,totalTax,basicFare,(isnull(TotalDiscount,0) + isnull(FlatDiscount,0) + isnull(PromoDiscount,0)) as TotalDiscount, 
...
select distinct(order_id),tracking_id,order_status,payment_mode,card_name, mer_amount,
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[FetchSelfReversaldata] TO [rt_read]
    AS [dbo];
