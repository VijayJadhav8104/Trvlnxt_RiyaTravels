...
--sp_helptext CheckBooking_UAT  
  
CREATE proc [dbo].[CheckBookingPending_UAT]    
@FlightNo varchar(10)    
,@Airline varchar(10)    
...
Begin    
 ...
 WHILE @@FETCH_STATUS = 0  
  ...
  SET @strCounter = CONVERT(VARCHAR(5),@Counter)  
   ...
   IF(@Counter = 1)  
   BEGIN  
    set @sQeury = 'SELECT vw_s1.fkBookMaster FROM (select tbi.fkBookMaster from tblBookItenary tbi WITH (NOLOCK) inner join tblbookmaster tbm WITH (NOLOCK) on tbi.fkBookMaster=tbm.pkid   
    where LTRIM(RTRIM(tbi.flightNo)) ='''+ LTRIM(RTRIM(@FlightNum)) +''' and (tbm.BookingStatus=0 or tbm.BookingStatus=3 or tbm.BookingStatus=2)) AS vw_s1'  
   END  
   ELSE     
   BEGIN  
    SET @sQeury = @sQeury +' inner join   
    (SELECT vw_s'+@strCounter+'.*  
    FROM (select tbi.fkBookMaster from tblBookItenary tbi WITH (NOLOCK) inner join tblbookmaster tbm WITH (NOLOCK) on tbi.fkBookMaster=tbm.pkid  
    where LTRIM(RTRIM(tbi.flightNo))='''+  LTRIM(RTRIM(@FlightNum)) +''' and (tbm.BookingStatus=0 or tbm.BookingStatus=3 or tbm.BookingStatus=2)) AS vw_s'+@strCounter+') as vw_p'+@strCounter+'  
...
FROM   
 tblBookMaster BM   WITH (NOLOCK)
 JOIN tblPassengerBookDetails PBD WITH (NOLOCK) ON PBD.fkBookMaster = BM.pkId   
 ...
 JOIN tblBookItenary BI WITH (NOLOCK) ON BI.fkBookMaster = BM.pkId  
WHERE   
