create procedure [dbo].[fetchERPData_NEW10] --fetchERPData_NEW2 '15-apr-2021','23-apr-2021',1,3,'in','3',''
...
,ROE,AgentROE,pmt.cardtype,pmt.cardnumber from tblPassengerBookDetails pax
left join tblBookMaster book on pax.fkBookMaster=book.pkId
left join Paymentmaster pmt on book.orderId=pmt.order_id
left join UserLogin ul on ul.UserID=book.LoginEmailID
...
CASE WHEN p.ticket1=p.ticket2 THEN isnull(p.TicketNumber,p.ticket1) ELSE isnull(p.TicketNumber,p.ticket1)+','+isnull(p.TicketNumber,p.ticket2) END as TicketNumber,
...
,pmt.cardtype,pmt.cardnumber,pax.TicketNumber
from tblPassengerBookDetails pax
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[fetchERPData_NEW10] TO [rt_read]
    AS [dbo];
