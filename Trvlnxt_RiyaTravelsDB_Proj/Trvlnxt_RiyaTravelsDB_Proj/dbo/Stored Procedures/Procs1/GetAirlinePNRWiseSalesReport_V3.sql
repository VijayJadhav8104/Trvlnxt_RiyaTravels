...
CREATE PROCEDURE [dbo].[GetAirlinePNRWiseSalesReport_V3] 
	@FromDate Date=NULL 
	, @ToDate Date=NULL
...
BEGIN 
	...
	SELECT * INTO #tempTableAirlinePNR FROM (
		...
		,(Select top 1 wy.[RH Ledgers] from tblInterBranchWinyatra wy 
			INNER JOIN B2BRegistration r1 WITH(NOLOCK)ON wy.Icust=r1.Icast
			INNER JOIN mVendorCredential mc WITH(NOLOCK) ON mc.OfficeId=b.OfficeID AND FieldName='CountryCode'  AND wy.Country=mc.Value
			) AS [RH Ledgers]
