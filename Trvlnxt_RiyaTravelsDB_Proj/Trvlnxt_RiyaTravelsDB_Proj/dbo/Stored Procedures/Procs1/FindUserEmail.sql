CREATE PROCEDURE FindUserEmail  -- FindUserEmail 0,27516,4257          
...
BEGIN          
 ...
 Begin          
  ...
  select @AlertEmailid=AlertEmail from Hotel_BookMaster where pkId = @pkid         
  select @AddrEmailid=b.AddrEmail  from AgentLogin A            
  inner join B2BRegistration B on a.UserID=B.FKUserID            
  where B.FKUserID= @UserId          
          
  select @MainAgentEmailid=mu.EmailID  from Hotel_BookMaster hb            
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[FindUserEmail] TO [rt_read]
    AS [dbo];
