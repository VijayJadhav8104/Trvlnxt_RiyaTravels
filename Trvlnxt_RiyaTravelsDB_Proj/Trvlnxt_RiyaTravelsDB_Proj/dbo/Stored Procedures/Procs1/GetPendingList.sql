...
CREATE procedure [dbo].[GetPendingList] 
@Userid int
as
begin
...
inner join tblPassengerBookDetails p on p.fkBookMaster=t.pkId
 ...
 AND T.Country in (select C.CountryCode  from mUserCountryMapping UM
			INNER JOIN mCountry C ON C.ID=UM.CountryId where UserID=@Userid  AND IsActive=1) and AgentAction=1
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[GetPendingList] TO [rt_read]
    AS [dbo];
