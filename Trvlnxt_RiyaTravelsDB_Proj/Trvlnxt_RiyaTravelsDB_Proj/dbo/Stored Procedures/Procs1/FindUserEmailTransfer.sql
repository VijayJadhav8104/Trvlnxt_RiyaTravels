CREATE PROCEDURE FindUserEmailTransfer  -- FindUserEmail 0,27516,4257                
...
BEGIN                
 ...
 Begin                
  ...
  select @AlertEmailid=PD.Email from TR.TR_BookingMaster   BR JOIN TR.TR_PaxDetails PD  on BR.BookingId = PD.BookingId where BR.BookingId = @pkid               
  select @AddrEmailid=b.AddrEmail  from AgentLogin A                  
  inner join B2BRegistration B on a.UserID=B.FKUserID                  
  where B.FKUserID= @UserId                
                
  select @MainAgentEmailid=mu.EmailID  from TR.TR_BookingMaster hb                  
