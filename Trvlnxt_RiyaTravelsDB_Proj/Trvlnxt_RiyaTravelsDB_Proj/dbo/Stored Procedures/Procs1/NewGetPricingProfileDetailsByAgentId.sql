 ...
 --  NewGetPricingProfileDetailsByAgentId 46999 ,null                                                   
-- Created By akash while doing APi for ZH api                                               
-- Created Date : 14-10-2022                                      
--=============================================================                                  
CREATE PROC NewGetPricingProfileDetailsByAgentId                                             
@AgentId INT  ,                           
...
BEGIN                                                      
 ...
 begin                          
  ...
  select PM.AgentId,PM.SupplierId,PM.ProfileId,                
  Case                 
    When PM.CancellationHours=null OR  PM.CancellationHours=0 then 24                   
 else PM.CancellationHours                
 end as CancellationHours                                     
  ...
  ,isnull(PM.ServiceChargeAmt,0) as 'ServiceChargeAmt'        
    ,isnull(PM.IsInterNationalChargesApplicable,0) as 'InternationalChargeApplicable'    
 ,SM.SupplierGroup  
 ,PM.IsHold  
 ...
 left join B2BRegistration BR on PM.AgentId=BR.PKID                        
  ...
  select CurrencyName from Hotel.VCCChargesByPassCurrency with(nolock) where IsActive=1        
