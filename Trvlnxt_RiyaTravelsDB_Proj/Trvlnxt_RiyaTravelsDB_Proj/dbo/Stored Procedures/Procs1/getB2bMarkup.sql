...
CREATE  proc [dbo].[getB2bMarkup] --'8/25/2021 12:50:00 AM','8/25/2021 12:50:00 AM','AE','40027','B2B'  
@Travelfrom datetime,  
@Travelto datetime,  
...
SELECT isnull(t.CategoryValue,'All') AS VendorName, B.ID as markupID, MarketPoint, AirportType, AirlineType, PaxType, Remark, OnBasic,  
...
,B.AgencyId as B2bAgentId,  
       ...
       ISNULL(displaytypem,0) AS displaytypem,ISNULL(bookingtypem,0) AS bookingtypem, TransactionType , AvailabilityPCC
  
FROM  
Flight_MarkupType B  
left join  Agent_Markup C on B.ID = C.MarkupID  
LEFT JOIN AgentLogin A ON A.UserID=c.AgentID
left join tbl_commonmaster t on t.pkid=b.CRSType
WHERE  
...
SaleValidityFrom <=  CONVERT(DATE,GETDATE()) and SaleValidityTo  >= CONVERT(DATE,GETDATE())  AND  
  ...
  and ( convert( varchar(50),@AgentID) in (select * from SplitString(B.AgencyId,',')) or Name='ALL')  
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[getB2bMarkup] TO [rt_read]
    AS [dbo];
