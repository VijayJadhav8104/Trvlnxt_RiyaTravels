...
create procedure [dbo].[fetchERPData_backup] --null,null,1,22
...
IF(@status=1)--status-Ticketed
	BEGIN
		...
		case when Vendor_No is not null then Vendor_No else(case WHEN P.airCode  in (SELECT AirlineCode FROM TblSTSAirLineMaster) then 'RABOM0300004' WHEN P.airCode  in (SELECT AirlineCode1 FROM TblSTSAirLineMaster) then 'RABOM0300004' ELSE '' END) end  as VendorNo,
		...
		FROM
			...
			,book.SupplierCode,book.Vendor_No
...
,pax.Markup,book.SupplierCode,book.Vendor_No
  ...
  END
	...
	BEGIN
		...
		FROM
			...
			pax.IATACommission AS IataCommissionC, book.VendorCommissionPercent	as IataCommissionV,pax.serviceCharge as MarkupOnTax
			,book.SupplierCode,book.Vendor_No,book.promoCode
			from tblPassengerBookDetails pax
			left join tblBookMaster book on pax.fkBookMaster=book.pkId
			left join Paymentmaster pmt on book.orderId=pmt.order_id
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[fetchERPData_backup] TO [rt_read]
    AS [dbo];
