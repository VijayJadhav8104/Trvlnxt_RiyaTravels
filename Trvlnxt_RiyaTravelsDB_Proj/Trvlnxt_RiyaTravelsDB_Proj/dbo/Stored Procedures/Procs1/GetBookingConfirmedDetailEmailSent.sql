...
CREATE PROCEDURE GetBookingConfirmedDetailEmailSent                                                                                                                               
...
BEGIN                                                                                                                            
   ...
   SM.Status as CurrentStatus,          
    (BR.AddrEmail+','+'tnsupport.hotels@riya.travel')  as 'AgencyEmail',           
  ('tn.hotels@riya.travel'+','+'TrvlNxt@riya.travel' ) as 'CcAddress',                
  ('harshit.gor@riya.travel'+','+'priti.kadam@riya.travel'+','+'gary.fernandes@riya.travel'+','+'developers@riya.travel' )    
   as 'BccAddress',               
            
  'ConfirmationAdded' as 'ConfStatus',              
   '' as VoucherID,                                                                                                                                   
...
------> Confirmation details                                                                                                                                  
 ...
 HB.CurrencyCode as 'Currency',                                                                                    
  ...
  left join Hotel_CountryMaster HC WITH (NOLOCK) on HB.BookingCountry=HC.CountryCode                                                                                              
  ...
  Left Join mUser MU1 WITH (NOLOCK) on HB.MainAgentID = MU1.ID                                                                          
  left join B2BHotel_Commission HComm WITH (NOLOCK) on HComm.Fk_BookId=HB.pkId                                           
...
-- passenger details                                                                                                                                                  
  select PAX.* from Hotel_Pax_master   as PAX                                                  
                                                                                                              
  INNER JOIN Hotel_BookMaster HB  WITH (NOLOCK) ON PAX.book_fk_id=HB.pkId                                                                                                            
                      
  where HB.pkId = @pkid                                                                                             
                                             
--- Room Details                                                                                                
  select  Room.* from Hotel_Room_master as Room                                                                                                            
                                                   
  INNER JOIN Hotel_BookMaster HB  WITH (NOLOCK) ON Room.book_fk_id=HB.pkId                                                                                                            
...
-- Room Rates Per night                                                                                            
  ...
  INNER JOIN Hotel_BookMaster HB  WITH (NOLOCK) ON RPN.FkBookingId=HB.pkId                                                                    
                                                                                         
  where HB.pkId = @pkid                                                                                                         
                                                                                                       
---- cancellation pollicy                                                                                       
                                                                                            
 select HBCP.* from  [Hotel].HotelCancelBookPolicies as HBCP                                                                            
    INNER JOIN Hotel_BookMaster HB  WITH (NOLOCK) ON HB.pkId=HBCP.FKBookingId                                           
  where HB.pkId = @pkid                                  
  ...
  isnull(GST.PinCode,'') as PinCode from  Hotel_BookingGSTDetails as GST                               
   INNER JOIN Hotel_BookMaster HB  WITH (NOLOCK) ON HB.pkId=GST.PKID                                                                                                
  where HB.pkId = @pkid                                                       
  select isnull(AC.Text,'') as Text, isnull(AC.Description,'') as Description,isnull(AC.Amount,0) as Amount from hotel.AdditionalCharges as AC                                                                          
  INNER JOIN Hotel_BookMaster HB  WITH (NOLOCK) ON AC.FkBookId=HB.pkId                                                                                                                                                      
  where HB.pkId = @pkid            
