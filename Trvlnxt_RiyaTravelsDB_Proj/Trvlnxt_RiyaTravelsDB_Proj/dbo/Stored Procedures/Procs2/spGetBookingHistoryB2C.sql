CREATE procEDURE [dbo].[spGetBookingHistoryB2C]  -- [dbo].[spGetBookingHistoryTest] 'TN.SUPPORT@RIYA.TRAVEL',4,'4IH3W6'
...
BEGIN
	...
	BEGIN
		...
		with cte as
			...
			SELECT a.pkId , a.airName as owairName,a.frmSector as owfrmSector,a.toSector as owtoSector,CONVERT(date,a.deptTime) as owdepDate,a.arrivalDate as owarrivalDate,a.fromAirport as owfromAirport,
				...
				,(SELECT Top 1 baggage FROM tblPassengerBookDetails pax2 WITH(NOLOCK)  WHERE a.pkid=pax2.fkBookMaster) as baggage
				,(SELECT Top 1 inserteddate FROM tblPassengerBookDetails pax3 WITH(NOLOCK)  WHERE  a.pkid=pax3.fkBookMaster) as BookingDate
				--,p.baggage as baggage,p.inserteddate as BookingDate
				,(SELECT TOP 1 pax.paxFName + ' '+pax.paxLName FROM tblPassengerBookDetails pax WITH(NOLOCK)  WHERE a.pkid=pax.fkBookMaster) as BookingBy,a.IsBooked as owIsBooked,b.IsBooked as rtIsBooked,
				a.toAirport,a.orderId,a.ROE,a.Country,a.OfficeID,a.TotalMarkup as confee,
				
				--Total Added By Jagdish
			...
			--,(SELECT TOP 1 Via FROM tblBookItenary BI WHERE BI.orderId=b.orderId) as ViaSector

			FROM tblBookMaster a WITH(NOLOCK) 
				left JOIN tblBookMaster b WITH(NOLOCK)  on a.orderId=b.orderId and a.GDSPNR=b.GDSPNR and b.returnFlag=1
				inner Join UserLogin u WITH(NOLOCK)  on a.LoginEmailID=u.UserID and u.UserName=@EmailID
			...
			SELECT b.pkId , b.airName as owairName,b.frmSector as owfrmSector,b.toSector as owtoSector,CONVERT(date,b.deptTime) as owdepDate,b.arrivalDate as owarrivalDate,b.fromAirport as owfromAirport,
				...
				case when a.canceledDate is null then 0 else 1 end as CancelFlag
				,(SELECT Top 1 baggage FROM tblPassengerBookDetails pax2  WITH(NOLOCK) WHERE b.pkid=pax2.fkBookMaster) as baggage
				,(SELECT Top 1 inserteddate FROM tblPassengerBookDetails pax3  WITH(NOLOCK) WHERE  b.pkid=pax3.fkBookMaster) as BookingDate
				--,p.baggage as baggage,p.inserteddate as BookingDate
				,(SELECT TOP 1 pax.paxFName + ' '+pax.paxLName FROM tblPassengerBookDetails pax WITH(NOLOCK)  WHERE b.pkid=pax.fkBookMaster) as BookingBy,
			...
			FROM tblBookMaster a WITH(NOLOCK) 
				inner JOIN tblBookMaster b WITH(NOLOCK) on a.orderId=b.orderId and a.GDSPNR!=b.GDSPNR 
				inner Join UserLogin u WITH(NOLOCK) on b.LoginEmailID=u.UserID and u.UserName=@EmailID
            where  b.returnFlag=1 and b.IsBooked=1 and  CONVERT(date,b.deptTime) >= GETDATE()
		)
		SELECT c.* INTO #TripUpcomingTempTable
             FROM cte c WHERE  CancelFlag=0 AND (owriyaPNR=@RiyaPNR or @RiyaPNR is null);

        select * from #TripUpcomingTempTable;

		SELECT c.paxFName,c.paxLName,c.pid,c.fkBookMaster,c.title,temp.owGDSPNR as GDSPNR,lTRIM(REPLACE(replace(SUBSTRING(c.ticketNum, 1, CHARINDEX('/', c.ticketNum)), 'PAX', ''),'/',''))as ticketNum,paxType as PaxType,c.BarCode
		
		--added by Jagdish
		,ISNULL((Select SSR_Name from tblSSRDetails as ssr WITH(NOLOCK) Where ssr.fkPassengerid=c.pid and ssr.SSR_Type='Baggage') ,'') as 'BaggageDesc'
		,ISNULL((Select SSR_Name from tblSSRDetails as ssr WITH(NOLOCK) Where ssr.fkPassengerid=c.pid and ssr.SSR_Type='Meals') ,'') as 'MealDesc'
		--,ViaSector
		FROM tblPassengerBookDetails c WITH(NOLOCK) 
	    inner join #TripUpcomingTempTable temp
		on c.fkBookMaster=temp.pkId
		order by c.paxType,c.ticketNum --added by bhavika
	...
	begin
			...
			SELECT a.pkId , a.airName as owairName,a.frmSector as owfrmSector,a.toSector as owtoSector,CONVERT(date,a.deptTime) as owdepDate,a.arrivalDate as owarrivalDate,a.fromAirport as owfromAirport,
				...
				,(SELECT Top 1 inserteddate FROM tblPassengerBookDetails pax3  WITH(NOLOCK) WHERE  a.pkid=pax3.fkBookMaster) as BookingDate
				--,p.baggage as baggage,p.inserteddate as BookingDate
				,(SELECT TOP 1 pax.paxFName + ' '+pax.paxLName FROM tblPassengerBookDetails pax WITH(NOLOCK)  WHERE a.pkid=pax.fkBookMaster) as BookingBy,a.IsBooked as owIsBooked,b.IsBooked as rtIsBooked,a.toAirport,a.orderId,a.ROE,a.Country,a.OfficeID,a.TotalMarkup as confee,
		...
		SELECT c.* INTO #TripCancelTempTable
             FROM cte c WHERE  CancelFlag=1 AND (owriyaPNR=@RiyaPNR or @RiyaPNR is null);
	...
	else if(@opr=4)  -- ManageBooking
	begin
	 	  	with cte as
			(
			
			SELECT a.pkId , a.airName as owairName,a.frmSector as owfrmSector,a.toSector as owtoSector,CONVERT(date,a.deptTime) as owdepDate,a.arrivalDate as owarrivalDate,a.fromAirport as owfromAirport,
				a.deptTime as owdeptTime,a.fromTerminal as owfromTerminal,a.toTerminal as owtoTerminal,(SELECT TOP 1 airlinePNR FROM tblBookItenary BI WHERE BI.fkBookMaster=a.pkId) as owGDSPNR,a.arrivalTime as owarrivalTime,
				a.flightNo as owflightNo,a.riyaPNR as owriyaPNR,a.isReturnJourney,a.emailId,a.mobileNo,a.returnFlag,
				CASE WHEN a.Country = 'IN' THEN CONVERT(VARCHAR(20),CONVERT(decimal(18,0), a.basicFare * a.ROE))ELSE CONVERT(VARCHAR(20),CONVERT(decimal(18,2),a.basicFare * a.ROE)) END as owbasicFare,--a.basicFare as owbasicFare,
				CASE WHEN a.Country = 'IN' THEN CONVERT(VARCHAR(20),CONVERT(decimal(18,0), a.totalTax * a.ROE))ELSE CONVERT(VARCHAR(20),CONVERT(decimal(18,2),a.totalTax * a.ROE)) END as owtotalTax,--a.totalTax as owtotalTax,
				CASE WHEN a.Country = 'IN' THEN CONVERT(VARCHAR(20),CONVERT(decimal(18,0), a.totalFare * a.ROE))ELSE CONVERT(VARCHAR(20),CONVERT(decimal(18,2),a.totalFare * a.ROE)) END as owtotalFare,--a.totalFare as owtotalFare,
				a.TotalTime as owTotalTime,a.airCode as owCarrier,a.CounterCloseTime as owCounterCloseTime,(a.FlatDiscount + a.PromoDiscount) as owTotalDiscount,
				b.airName as rtairName,b.frmSector as rtfrmSector,b.toSector as rttoSector,CONVERT(date,b.deptTime) as rtdepDate,b.arrivalDate as rtarrivalDate,
				b.deptTime as rtdeptTime,b.arrivalTime as rtarrivalTime,b.flightNo as rtflightNo,b.riyaPNR as rtriyaPNR,
				CASE WHEN b.Country = 'IN' THEN CONVERT(VARCHAR(20),CONVERT(decimal(18,0), b.basicFare * a.ROE))ELSE CONVERT(VARCHAR(20),CONVERT(decimal(18,2),b.basicFare * a.ROE)) END as rtbasicFare,--b.basicFare as rtbasicFare,
				CASE WHEN b.Country = 'IN' THEN CONVERT(VARCHAR(20),CONVERT(decimal(18,0), b.totalTax * a.ROE))ELSE CONVERT(VARCHAR(20),CONVERT(decimal(18,2),b.totalTax * a.ROE)) END as rttotalTax,--b.totalTax as rttotalTax, 
				CASE WHEN b.Country = 'IN' THEN CONVERT(VARCHAR(20),CONVERT(decimal(18,0), b.totalFare * a.ROE))ELSE CONVERT(VARCHAR(20),CONVERT(decimal(18,2),b.totalFare * a.ROE)) END as rttotalFare,--b.totalFare as rttotalFare,
				b.fromTerminal as rtfromTerminal,b.toTerminal as rttoTerminal,b.CounterCloseTime as rtCounterCloseTime,(b.FlatDiscount + b.PromoDiscount) as rtTotalDiscount,
				...
				,(SELECT Top 1 inserteddate FROM tblPassengerBookDetails pax3  WITH(NOLOCK) WHERE  a.pkid=pax3.fkBookMaster) as BookingDate
				--,p.baggage as baggage,p.inserteddate as BookingDate
				,(SELECT TOP 1 pax.paxFName + ' '+pax.paxLName FROM tblPassengerBookDetails pax  WITH(NOLOCK) WHERE a.pkid=pax.fkBookMaster) as BookingBy,a.IsBooked as owIsBooked,b.IsBooked as rtIsBooked,a.toAirport,a.orderId,a.ROE,a.Country,a.OfficeID
			,a.TotalMarkup as confee,
			...
			SELECT b.pkId , b.airName as owairName,b.frmSector as owfrmSector,b.toSector as owtoSector,CONVERT(date,b.deptTime) as owdepDate,b.arrivalDate as owarrivalDate,b.fromAirport as owfromAirport,
				b.deptTime as owdeptTime,b.fromTerminal as owfromTerminal,b.toTerminal as owtoTerminal,(SELECT TOP 1 airlinePNR FROM tblBookItenary BI WHERE BI.fkBookMaster=b.pkId) as owGDSPNR,b.arrivalTime as owarrivalTime,
				b.flightNo as owflightNo,b.riyaPNR as owriyaPNR,b.isReturnJourney,b.emailId,b.mobileNo,b.returnFlag,
				CASE WHEN b.Country = 'IN' THEN CONVERT(VARCHAR(20),CONVERT(decimal(18,0), b.basicFare * a.ROE))ELSE CONVERT(VARCHAR(20),CONVERT(decimal(18,2),b.basicFare * a.ROE)) END as owbasicFare,--b.basicFare as owbasicFare,
				CASE WHEN b.Country = 'IN' THEN CONVERT(VARCHAR(20),CONVERT(decimal(18,0), b.totalTax * a.ROE))ELSE CONVERT(VARCHAR(20),CONVERT(decimal(18,2),b.totalTax * a.ROE)) END as owtotalTax,--b.totalTax as owtotalTax,
				CASE WHEN b.Country = 'IN' THEN CONVERT(VARCHAR(20),CONVERT(decimal(18,0), b.totalFare * a.ROE))ELSE CONVERT(VARCHAR(20),CONVERT(decimal(18,2),b.totalFare * a.ROE)) END as owtotalFare,--b.totalFare as owtotalFare,
				b.TotalTime as owTotalTime,b.airCode as owCarrier,b.CounterCloseTime as owCounterCloseTime,(b.FlatDiscount + b.PromoDiscount) as owTotalDiscount,
		...
		SELECT c.* INTO #ManageBookingTempTable
             FROM cte c

        select * from #ManageBookingTempTable;
		...
		FROM tblPassengerBookDetails c WITH(NOLOCK) 
	    inner join #ManageBookingTempTable temp		
		on c.fkBookMaster=temp.pkId
		order by c.paxType,c.ticketNum --added by bhavika

		drop table #ManageBookingTempTable; 
	end
END