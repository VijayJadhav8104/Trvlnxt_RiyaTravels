CREATE procEDURE [dbo].[spGetBookingHistoryTest]  -- spGetBookingHistoryTest 'sushil.agarwal161@gmail.com',1,null
...
BEGIN
	...
	BEGIN
		...
		with cte as
			...
			FROM tblBookMaster a
				...
				inner Join UserLogin u on a.LoginEmailID=CONVERT(varchar(10), u.UserID) and u.UserName=@EmailID
			...
			FROM tblBookMaster a
				...
				inner Join UserLogin u on b.LoginEmailID=CONVERT(varchar(10), u.UserID) and u.UserName=@EmailID
	...
	begin
			...
			SELECT a.pkId , a.airName as owairName,a.frmSector as owfrmSector,a.toSector as owtoSector,CONVERT(date,a.deptTime) as owdepDate,a.arrivalDate as owarrivalDate,a.fromAirport as owfromAirport,
				...
				,(SELECT Top 1 inserteddate FROM tblPassengerBookDetails pax3 WHERE  a.pkid=pax3.fkBookMaster) as BookingDate
				--,p.baggage as baggage,p.inserteddate as BookingDate
				,(SELECT TOP 1 pax.paxFName + ' '+pax.paxLName FROM tblPassengerBookDetails pax WHERE a.pkid=pax.fkBookMaster) as BookingBy,a.IsBooked as owIsBooked,b.IsBooked as rtIsBooked,a.toAirport,a.orderId,a.ROE,a.Country,a.OfficeID,a.TotalMarkup as confee
			FROM tblBookMaster a
				left JOIN tblBookMaster b on a.orderId=b.orderId and a.GDSPNR=b.GDSPNR and b.returnFlag=1
				inner Join UserLogin u on a.LoginEmailID=CONVERT(varchar(10), u.UserID) and u.UserName=@EmailID
	...
	begin
			...
			SELECT a.pkId , a.airName as owairName,a.frmSector as owfrmSector,a.toSector as owtoSector,CONVERT(date,a.deptTime) as owdepDate,a.arrivalDate as owarrivalDate,a.fromAirport as owfromAirport,
				...
				,(SELECT Top 1 inserteddate FROM tblPassengerBookDetails pax3 WHERE  a.pkid=pax3.fkBookMaster) as BookingDate
				--,p.baggage as baggage,p.inserteddate as BookingDate
				,(SELECT TOP 1 pax.paxFName + ' '+pax.paxLName FROM tblPassengerBookDetails pax WHERE a.pkid=pax.fkBookMaster) as BookingBy,a.IsBooked as owIsBooked,b.IsBooked as rtIsBooked,a.toAirport,a.orderId,a.ROE,a.Country,a.OfficeID
			,a.TotalMarkup as confee FROM tblBookMaster a
				left JOIN tblBookMaster b on a.orderId=b.orderId and isnull(a.GDSPNR,'')=isnull(b.GDSPNR,'') and b.returnFlag=1
			...
			SELECT b.pkId , b.airName as owairName,b.frmSector as owfrmSector,b.toSector as owtoSector,CONVERT(date,b.deptTime) as owdepDate,b.arrivalDate as owarrivalDate,b.fromAirport as owfromAirport,
				...
				b.IsBooked as owIsBooked,b.IsBooked as rtIsBooked,b.toAirport,a.orderId,a.ROE,a.Country,a.OfficeID,a.TotalMarkup as confee
			FROM tblBookMaster a
				inner JOIN tblBookMaster b on a.orderId=b.orderId and isnull(a.GDSPNR,'')!=isnull(b.GDSPNR ,'')
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[spGetBookingHistoryTest] TO [rt_read]
    AS [dbo];
