...
--exec TrvlNxt_SearchprocessdataAgentByStatusNEW2 '2022-10-08','2022-10-11','','3UUG23','','','',99,99,'','',5000,0,0,0,3,'','',3,0 
CREATE PROCEDURE [dbo].[TrvlNxt_SearchprocessdataAgentByStatusNEW2] 
  @FROMDate Date = null, 
  @ToDate Date = null, 
  @paxname varchar(100)= '', 
  @PNR varchar(20)= '', 
...
set 
  ...
  BEGIN
  if (@Status = 99) BEGIN IF OBJECT_ID ('tempdb..#tempTableA') IS NOT NULL   
...
from 
  (
    ...
    (
      (
        select 
          Isnull(
            SUM(
              totalfare + isnull(MCOAmount, 0) --+ ISNULL(ServiceFee,0) + ISNULL(GST,0)
              + isnull(BFC, 0) + isnull(TotalHupAmount, 0)
            ), 
            0
          ) 
        from 
          tblBookMaster(nolock) 
        where 
          riyaPNR = a.RiyaPNR
    ...
    WHERE 
      (
        a.returnFlag = 0 and  a.AgentID !='B2C' and a.riyaPNR='3UUG23'    
