   ...
   -- exec SameDay_Hotel_Booking_8  
  
CREATE proc [dbo].[SameDay_Hotel_Booking_8]            
as             
   declare @countno int  
begin  
...
if (@countno>0)  
  ...
  --      -------------------------------------------
        ...
        WITH RankedData AS (
            ...
            WHERE 
		         (AgencyId IN(select CASE WHEN MainAgentID = 0 THEN RiyaAgentID ELSE 0 END from #TempBookingData) or
		         UserID IN (SELECT CASE WHEN MainAgentID > 0 THEN MainAgentID ELSE 0 END FROM #TempBookingData))
				  and (UserLoginCountry not like '%Exception occured%' 
				  or UserLoginCountry is null)
