...
CREATE PROCEDURE [dbo].[UpdateCheckBalance_Console]        
(        
    @UserId INT,        
...
BEGIN TRAN     
 ...
 DECLARE @BookingStatus VARCHAR(50);   
    ...
    FROM tblAgentBalance WITH (XLOCK)         
  ...
  SELECT @TransactionPkId= SCOPE_IDENTITY()    SELECT TOP 1 @BookingStatus= ISNULL(HSM.Status,'')   FROM Hotel_Status_History HSH   JOIN Hotel_BookMaster BM ON BM.pkId=HSH.FKHotelBookingId   JOIN Hotel_Status_Master HSM ON HSH.FkStatusId=HSM.Id   
  WHERE orderId=@BookingRef ORDER BY HSH.CreateDate DESC    INSERT INTO hotel.Agentbalance_StatusLog(FKtransactionID,BookingStatus) VALUES (@TransactionPkId,@BookingStatus)  
 ...
 insert into ExceptionLog(ErrorMessage,ErrorProcedure,ErrorNumber) values(@BookingRef,@ModelID,@UserId)     
