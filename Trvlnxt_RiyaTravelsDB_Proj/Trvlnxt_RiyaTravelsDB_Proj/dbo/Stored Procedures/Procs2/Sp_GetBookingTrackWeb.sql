--exec [dbo].[Sp_GetBookingTrackWeb] '2023-05-11 00:00','2023-05-11 23:59','','1,2,3,4,5','IN,US,CA,AE,UK',45325,0,'3','','','','','Amadeus','','',''
CREATE PROCEDURE [dbo].[Sp_GetBookingTrackWeb]          
...
       when  country.CountryCode='CA' then (DATEADD(SECOND, - 9*60*60 -29*60 -16,CONVERT(varchar(20),CONVERT(DATETIME, isnull(b.inserteddate_old,b.inserteddate)),120)))           
		WHEN country.CountryCode='GB' THEN (DATEADD(SECOND, - 5*60*60 + 30*60, CONVERT(varchar(20), CONVERT(DATETIME, ISNULL(b.inserteddate_old,b.inserteddate)), 120)))
...
-- (select SUM (cast((((bm.totalFare+isnull(bm.TotalMarkup,0)+isnull(bm.ServiceFee,0)+isnull(bm.GST,0)+isnull(bm.AgentMarkup,0))-isnull(bm.TotalDiscount,0))* bm.ROE * bm.AgentROE + isnull(bm.B2BMarkup,0)) as decimal(18,2))) from tblBookMaster as bm where bm.GDSPNR = b.GDSPNR AND bm.riyaPNR = b.riyaPNR) AS 'Net fare'             
--,(select SUM(cast(((bm.totalFare+isnull(bm.ServiceFee,0)+isnull(bm.GST,0)+isnull(bm.AgentMarkup,0))* bm.ROE * bm.AgentROE + bm.B2BMarkup) as decimal(18,2))) From tblBookMaster as bm where bm.GDSPNR = b.GDSPNR AND bm.riyaPNR = b.riyaPNR)  + (Select SUM(SSR_Amount)* b.ROE * b.AgentROE  from tblSSRDetails as ssr Where ssr.fkBookMaster IN (Select pkid From tblBookMaster where GDSPNR = b.GDSPNR AND riyaPNR = b.riyaPNR)) as 'Gross fare'
...
b.Remarks as 'UpdatedUserRemarks',          
 ...
 when b.MainAgentId=0 and b.BookingSource='Retrive PNR - MultiTST' then 'Agent Booking (Retrive PNR - MultiTST)'           
   ...
   when b.MainAgentId>0 and b.BookingSource='ManualTicketing' then 'Manual Booking'   

   when b.MainAgentId=0 and b.BookingSource='Retrive PNR Accounting' then  'Agent Booking (Retrive PNR Accounting)'
   when b.MainAgentId>0 and b.BookingSource='Retrive PNR Accounting' then 'Internal Booking (Retrive PNR Accounting)'

   when b.MainAgentId>0 and b.BookingSource='Retrieve PNR accounting - MultiTST' then 'Retrieve PNR accounting - MultiTST' end) as 'Booking Mode',          
 ...
 ,b.inserteddate ,b.orderId ,b.riyaPNR ,b.AgentId         
  ,TFBookingRef as TFrefNumber,TFBookingstatus       
 from tblBookMaster b          
 left JOIN agentLogin AL ON cast(AL.UserID AS VARCHAR(50))=B.AgentID          
 ...
 left join mCountry country on b.Country=country.CountryCode          
     
           
 where ((@FROMDate = '') or (CONVERT(datetime,b.IssueDate) >= CONVERT(datetime,@FROMDate)))          
    AND ((@ToDate = '') or (CONVERT(datetime,b.IssueDate) <= CONVERT(datetime, @ToDate)))          
    AND ((@PaymentType = '') or (@PaymentType='Wallet' and (p.payment_mode='Check' or p.payment_mode='Credit')) or (p.payment_mode=@PaymentType))          
 ...
 or((@BookingType='ABRA') and b.MainAgentId>0 and (b.BookingSource='Retrieve PNR accounting - MultiTST'))      
    ...
    and b.returnFlag=0    and b.pkId in (select top 1 pkid from tblBookMaster where orderid=b.orderId order by pkId asc)      
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[Sp_GetBookingTrackWeb] TO [rt_read]
    AS [dbo];
