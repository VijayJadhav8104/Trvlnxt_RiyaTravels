Create PROCEDURE [dbo].[USP_ERP_Activity_Rakhi29012025] --'CABookingtickets','','WGZVFE-1',''  
...
GSTonPLB,0) as decimal(18,2)) + CAST(isnull(PB.tdsonplb ,0) as decimal(18,2))) as PLBCommission,  
 ...
 IF @Action = 'B2BBookingTickets'          
  ...
  case when BM.VendorName='TravelFusion' then 'IN' else ED.OwnerCountry END AS OwnerCountry,PB.pid,  
   ...
   ISNULL(BM.ValidatingCarrier,BM.airCode) as ValidatingCarrier, PB.IATACommission as CustIATACommAmount ,(Cast(isnull(PB.PLBCommission,0) as decimal(18,2)) + Cast(isnull(PB.DropnetCommission,0) as decimal(18,2)) + CAST(isnull(PB.GSTonPLB,0) as decimal(18
,2)) + CAST(isnull(PB.tdsonplb ,0) as decimal(18,2))) as PLBCommission, BBM.TotalCommission as Surcharge,  
   --case when AC.type = 'FSC' then BM.GDSPNR else BI.airlinePNR end as PNRNo,  
   case when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) < 10then 'BOMVEND007913'   
   when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'IN' then 'VEND000178'  
   when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'AE' then 'VEND00102'  
 ...
 IF @Action = 'RBTBookingTickets'          
  ...
  --as MealCharges,  
   ...
   BM.pkId,
   BM.riyaPNR,
   BM.inserteddate,BM.IssueDate,BM.CounterCloseTime, PB.IATACommission as CustIATACommAmount ,
   (Cast(isnull(PB.PLBCommission,0) as decimal(18,2)) + Cast(isnull(PB.DropnetCommission,0) as decimal(18,2)) + CAST(isnull(PB.GSTonPLB,0) as decimal(18,2)) + CAST(isnull(PB.tdsonplb ,0) as decimal(18,2))) as PLBCommission,  
 ...
 IF @Action = 'RBT-US-CABookingTickets'          
  ...
  when AL.GroupId = '21' then isnull(replace(ATT.Issuancedate,'&','and'),'')  
  else  
  ISNULL(ATT.Travelduration,'') end as _str1054,  
  case when AL.GroupId = '21' then isnull(replace(ATT.BillingEntityName,'&','and'),'') 
  when al.GroupId='3' then ISNULL(ATT.RequestID,'')
  when AL.GroupId = '18' then isnull(replace(ATT.TravelReason,'&','and'),'') 
  else  
  ISNULL(ATT.TASreqno,'') end as _str1022, ISNULL(ATT.Companycodecc,'')  as _str1023, 
  case when AL.GroupId = '10' then '' 
  when AL.GroupId = '18' then isnull(replace(ATT.TRAVELCOSTREIMBURSABLE,'&','and'),'')
  else ISNULL(Replace(ATT.Projectcode,'&','&amp;'),'') end as _str1024,  
 ...
 IF @Action = 'RBT-US-CACancellationTickets'          
   BEGIN          
    ...
    case when BM.BookingSource = 'Retrive PNR Accounting' THEN PB.FMCommission   
       ...
       THEN PB.Markup  
       ELSE PB.Markup END AS VendIATACommAmount,isnull(PB.VendorServiceFee,'0.00') as VendorServiceFee,  
    ISNULL(BM.ValidatingCarrier,BM.airCode) as ValidatingCarrier, PB.IATACommission as CustIATACommAmount ,(Cast(isnull(PB.PLBCommission,0) as decimal(18,2)) + Cast(isnull(PB.DropnetCommission,0) as decimal(18,2)) + CAST(isnull(PB.GSTonPLB,0) as decimal(
18,2)) + CAST(isnull(PB.tdsonplb ,0) as decimal(18,2))) as PLBCommission, PB.CancelledDate as PostingDate,  
...
ON ( 
  ...
  as MealCharges,  
   ...
   FROM tblBookMaster BM WITH (NOLOCK)  
    ...
    LEFT Join B2BMakepaymentCommission BBM WITH (NOLOCK) on BBM.orderId = BM.OrderId  
    WHERE BM.IsBooked =1 and BM.totalFare > 0 and BM.AgentID != 'B2C'       
    and (BM.Isfakebooking = 0 or BM.Isfakebooking is null)      and BM.BookingStatus In (1,6) --= 4 For Cancellation  =1 For Booking  --As confirm with Mansavee          
    -- For Booking          
    --and isnull(BM.ERPPush,0) != 0  
 ...
 IF @Action = 'UAEBookingTickets'          
   ...
   PB.WOTax,PB.INTax,(PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax',PB.OCTax,PB.ExtraTax,PB.BaggageFare,PM.CardType, PB.YMTax,  
   PM.MaskCardNumber as 'CardNumber',PM.mer_amount,PM.billing_address,PM.billing_name,PM.order_id,PM.tracking_id,--BI.airlinePNR,BI.cabin,BI.farebasis,          
