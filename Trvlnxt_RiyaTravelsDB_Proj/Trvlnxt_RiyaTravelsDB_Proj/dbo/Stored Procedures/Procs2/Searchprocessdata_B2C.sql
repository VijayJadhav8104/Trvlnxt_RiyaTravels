...
--declare @recordCount int =50
--Searchprocessdata_B2C '2024-06-18 00:00:00','2024-06-18 23:59:59','','','','','','All','All','','',0,50,68,'',null,'',null,1,NULL
CREATE PROCEDURE [dbo].[Searchprocessdata_B2C] --[Searchprocessdata_B2C] '2024-05-16','2024-05-16','','','','','','All',null,'','',0,500,3,null,null,'B2C','IN'
	...
	BEGIN
		...
		SELECT * INTO #tempTableA FROM( 
			...
			, (case when a.AgentID='B2C' THEN '-' when (SELECT CAST(ParentAgentID AS VARCHAR(50)) FROM agentLogin WITH(NOLOCK) WHERE CAST(UserID AS VARCHAR(50))=a.AgentID) IS NULL THEN 'Main' 
					...
					END) AS AgencyType
			--added by Riya
			,CASE WHEN (LG.IsSSOUser = 1) THEN 'Staff' WHEN LG.IsSSOUser IS NULL OR LG.IsSSOUser = '' THEN 'B2C' ELSE 'B2C' END AS Source
			...
			LEFT JOIN Userlogin LG ON a.LoginEmailID = CONVERT(VARCHAR(50),LG.UserID)
