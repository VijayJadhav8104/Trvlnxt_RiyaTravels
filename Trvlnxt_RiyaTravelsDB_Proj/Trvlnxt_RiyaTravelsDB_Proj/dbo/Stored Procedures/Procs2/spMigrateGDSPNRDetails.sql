CREATE proc spMigrateGDSPNRDetails
...
if exists(select * from tblBookMaster where GDSPNR=@GDSPNR and (isbooked=0 OR isbooked IS NULL))
begin
...
delete from tblbookmaster where GDSPNR=@GDSPNR and (isbooked=0 OR isbooked IS NULL  )
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[spMigrateGDSPNRDetails] TO [rt_read]
    AS [dbo];
