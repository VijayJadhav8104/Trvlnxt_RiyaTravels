CREATE PROCEDURE [dbo].[SearchprocessdataAgentByStatusNEW3] 
...
from
	(  
	  select  a.BookingSource,pid, ISNULL(paxfname,'')+' '+paxlname as paxname, a.emailId as email, a.mobileNo as mob,
	        a.RiyaPNR, a.airName, 
	 tbi.airlinePNR,
			-- a.frmsector, a.tosector AS tosector, 
			a.deptTime AS deptdate, a.arrivaldate AS arrivaldate, a.OrderId, (a.totalfare + isnull(a.MCOAmount,0) + ISNULL(A.ServiceFee,0) + ISNULL(A.GST,0) + isnull(a.BFC,0)) as totalfare,isnull(a.ROE,1) ROE,isnull(a.AgentROE,1
...
--	AS AllPax FROM tblBookItenary t where t.fkBookMaster=a.pkId  and t.frmSector=a.frmSector  GROUP BY t.fkBookMaster,airlinePNR)
			 ...
			 from
			 (select * from 
(select pid, paxFName,paxLName,fkBookMaster,ROW_NUMBER() OVER(Partition by fkBookMaster ORDER BY pid) AS RN
...
-- and a.BookingStatus=1 
	...
	--	AND a.Country in (select country from UserCountryMapping where UserID=@Userid  AND IsActive=1)
			 ...
			   
		 --AND 
		 --(a.AgentID = CAST(@User	id AS VARCHAR(50))  or   a.AgentID in (select cast (userid as varchar(50)) from AgentLogin where ParentAgentID=@Userid))
	   ) p
...
AgentROE,isnull(a.B2BMarkup,0) B2BMarkup,isnull(a.AgentMarkup,0) Markup,
			...
			when a.BookingStatus=3 then 'Pending Ticket'
			when a
.BookingStatus=4 then 'Cancel'
			when a.BookingStatus=5 then 'Close'
	 
			END AS [Ticketstatus],
			case when a.BookingStatus=1 then 'Bookticket'
			END AS [Bookticket],			 c.tracking_id,a.Country,c.currency--a.OfficeID
...
--	  GROUP BY t.fkBookMaster,airlinePNR) as 'airlinePNR'
			 ...
			 from (select * from 
(select pid, 
paxFName,paxLName,fkBookMaster,
...
 a
	 ...
	       ON a.pkId=t1.fkBookMaster
		 ...
		   ON a
.pkId=tbi.fkBookMaster and a.frmSector=tbi.frmSector
		   LEFT JOIN Paymentmaster c
	       ON a.OrderId=c.order_id
		   
		   
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[SearchprocessdataAgentByStatusNEW3] TO [rt_read]
    AS [dbo];
