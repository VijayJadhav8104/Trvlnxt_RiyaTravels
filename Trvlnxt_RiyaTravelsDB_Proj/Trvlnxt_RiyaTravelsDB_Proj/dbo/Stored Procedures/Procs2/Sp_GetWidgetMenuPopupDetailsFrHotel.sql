--Sp_GetWidgetMenuPopupDetailsFrHotel 21958
CREATE PROCEDURE Sp_GetWidgetMenuPopupDetailsFrHotel                  
    @AgentId int = NULL                  
AS                  
BEGIN                  
  ...
  isnull(ReferenceID,0) as ReferenceID,
   ...
   OccupancyAsJson,
     isnull(ReferenceID,0) as ReferenceID,
        ...
        FROM HotelSearchAvailablityData                  
    ...
    SELECT             
        ...
        MAX(Lat) AS Lat,                  
        MAX(Long) AS Long,               
        MAX(AdultCount) AS AdultCount,            
        MAX(ChildCount) AS ChildCount,            
        MAX(InsertedDate) AS InsertedDate,            
        MAX(CheckIn) AS CheckIn,                
        MAX(CheckOut) AS CheckOut,                
        MAX(RoomCount) AS RoomCount,            
        MAX(SearchType) AS SearchType,            
        MAX(Residence) AS Residence,           
  MAX(OccupancyAsJson) AS OccupancyAsJson, 
  MAX(ReferenceID) AS ReferenceID,
