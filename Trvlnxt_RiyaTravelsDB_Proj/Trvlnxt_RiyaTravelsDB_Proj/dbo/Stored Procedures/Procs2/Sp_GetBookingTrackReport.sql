CREATE PROCEDURE [dbo].[Sp_GetBookingTrackReport] 
...
   	   when   country.CountryCode='US' then(DATEADD(SECOND, - 9*60*60 -29*60 
-16,CONVERT(varchar(20),CONVERT(DATETIME, isnull(b.inserteddate_old,b.inserteddate)),120))) 
   	   when  country.CountryCode='CA' then (DATEADD(SECOND, - 9*60*60 -29*60 -16,CONVERT(varchar(20),CONVERT(DATETIME, isnull(b.inserteddate_old,b.inserteddate)),
120))) 
...
    end   AS 'Booking date & time',
	...
	(CASE WHEN (b.BookingStatus=1 or b.BookingStatus=6) THEN 'Confirmed' WHEN B.BookingStatus=2 THEN 'Hold' WHEN B.BookingStatus=3 THEN 'Pending'
			...
			WHEN B.BookingStatus=10 THEN 'Manual Booking' WHEN B.BookingStatus=8 THEN 'Reschedule PNR' WHEN B.BookingStatus=12 THEN 'In-Process'  END) AS 'Status',	B.riyaPNR AS 'Booking id',
...
country.Currency as 'Currency',
	...
	convert(varchar,isnull( cast((((B.totalFare+B.TotalMarkup+B.MCOAmount+b.AgentMarkup)-B.TotalDiscount)* b.ROE+B.B2BMarkup) as decimal(18,2)) ,0)) AS 'Net fare',
	B.MCOAmount as 'MCO Amount',
	cast(((b.totalFare+isnull(b.MCOAmount,0) + +isnull(b.AgentMarkup,0))* b.ROE+B.B2BMarkup) as decimal(18,2)) as 'Gross fare',
	(SELECT CONVERT(VARCHAR(10), CONVERT(DATETIME, b.BookingTrackModifiedOn, 0), 103) 
+ ' '+ CONVERT(VARCHAR(10),CONVERT(DATETIME, b.BookingTrackModifiedOn, 0), 108)) as 'UpdatedDate',
(u.UserName + '-' + u.FullName) as 'UpdatedbyUser',b.Remarks as 'UpdatedUserRemarks',
	...
	(case when b.MainAgentId>=0 and b.BookingSource='Web' then 'Internal Booking (Web)'
			...
			when b.MainAgentId=0 and b.BookingSource='Cryptic' then 'Agent Booking (Cryptic)'end) as 'Booking Mode',
...
 		 AND ((@PaymentType = '') or (@PaymentType='Wallet' and (p.payment_mode='Check' or p.payment_mode='Credit')) or (p.payment_mode=@PaymentType))
		 ...
		 AND ( (@BookingType = '') or((@BookingType='IBW') 
and b.MainAgentId>0 and (b.BookingSource='Web'))
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[Sp_GetBookingTrackReport] TO [rt_read]
    AS [dbo];
