...
CREATE proc [dbo].[spInsertPaymentDetails]                
  ...
  ,@AuthCode varchar(50)=null              
   ...
   ,@IsHold bit = null    
   ,@AmendmentRefNo varchar(30)=null
...
ELSE                
BEGIN                
 Update Paymentmaster set order_status = @order_status,amount=@amount,mer_amount=@mer_amount WHERE order_id=@order_id                
 insert into [dbo].[Paymentissuance]([Tracking_Id],[Amount],[OrderId],[Status],Type)                 
                           values(@tracking_id,@amount,@order_id,@order_status,@type)                
END                
  COMMIT TRANSACTION;                
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[spInsertPaymentDetails] TO [rt_read]
    AS [dbo];
