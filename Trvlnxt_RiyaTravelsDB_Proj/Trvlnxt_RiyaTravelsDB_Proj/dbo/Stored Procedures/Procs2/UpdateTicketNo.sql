CREATE PROCEDURE [dbo].[UpdateTicketNo] --exec UpdateTicketNo 433762 , '0983906290519C1' , '','','0' ,'1'                           
@PaxId      int,                             
...
SET ticketNum = @FullTicketNumber,TicketNumber=@newticketno,MCOAmount=@MCOAmount      
,MCOTicketNo=@McoTicketNumber, baggage = case when @baggage is not null AND @baggage <> '' then @baggage else baggage end         
WHERE pid = @PaxId         
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[UpdateTicketNo] TO [rt_read]
    AS [dbo];
