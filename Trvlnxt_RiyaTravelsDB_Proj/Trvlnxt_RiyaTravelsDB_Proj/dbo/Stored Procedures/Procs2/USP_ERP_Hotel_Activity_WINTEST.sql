--sp_helptext USP_ERP_Hotel_Activity 'Hotel_WinYatraBooking'        
--=================================        
...
CREATE PROCEDURE [dbo].[USP_ERP_Hotel_Activity_WINTEST]                                                           
...
ORDER BY HBM.pkId DESC                                                          
 ...
 IF @Action = 'HotelCancellation'                                                             
  ...
  case when HBM.BookingCountry = 'AE' then 'VEND00137' WHEN HBM.BookingCountry = 'SA' THEN 'VEND00024' else 'VEND000151' end as 'PayToVendor',                                                 
...
SELECT               
 ...
 and ((AL.userTypeID = 2 and B2BR.Icast in (select HotelIcust from tblInterBranchWinyatra)) or AL.userTypeID IN(4))                                                        
    --AND DATEDIFF(MINUTE,CAST( HB.inserteddate AS datetime),GETDATE())>5                      
      AND CAST( HB.inserteddate  AS DATE )>= CAST('2024-12-06' AS DATE)                      
 and isnull(HB.winyatraInvoice ,'')=''                      
   and HB.BookingReference in ('TNH00271019','TNH00271021')    
