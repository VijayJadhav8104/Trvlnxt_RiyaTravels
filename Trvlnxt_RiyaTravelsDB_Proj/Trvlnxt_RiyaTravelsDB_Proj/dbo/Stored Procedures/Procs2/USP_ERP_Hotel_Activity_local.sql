...
CREATE PROCEDURE [dbo].[USP_ERP_Hotel_Activity_local]                                                                                             
...
WHERE HSHVouchred.FkStatusId = 4                                                                      
 ...
 -- )                                                                                            
and  
 HBM.BookingReference  in(  
 select value   
 from dbo.fn_split('TNH00272020',','))                                                                                            
ORDER BY HBM.pkId DESC                                                                                            
 ...
 WHEN HBM.BookingCountry = 'SA' THEN 'VEND00024'                           
  --US, CA integration                             
   WHEN HBM.BookingCountry = 'US'                                                                            
   THEN 'UVEND01021'                                        
   WHEN HBM.BookingCountry = 'CA'                                                                           THEN 'CVEND00242'                           
...
and HBM.B2BPaymentMode=2                                                                                            
  ...
  Order by HBM.pkId desc                                                                                            
   ...
   BEGIN                                    
       SELECT                                                   
  TOP 1000 HB.pkId,HB.BookingReference,HB.Hotel_ERPResponceID      
  ,HB.AgentCommission      
  ,MU.FullName + '-' + MU.UserName as 'UserName'      
,BW.BranchId,BW.SubLed,HB.cityName,HB.Hotel_ERPPushStatus,HB.AgentId,HB.RiyaAgentID,AL.userTypeID      
  ,HB.inserteddate      
   ,HSHVouchred.CreateDate as 'PostingDate' --#1044-PostingDate Should Status Change Date      
