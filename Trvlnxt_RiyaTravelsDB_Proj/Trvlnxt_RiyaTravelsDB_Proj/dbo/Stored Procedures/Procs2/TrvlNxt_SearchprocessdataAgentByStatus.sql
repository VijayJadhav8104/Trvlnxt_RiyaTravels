...
CREATE PROCEDURE [dbo].[TrvlNxt_SearchprocessdataAgentByStatus]
	@FROMDate Date = NULL  
	, @ToDate Date = NULL  
...
BEGIN
		...
		BEGIN  		 
				...
				SELECT A.Country
						...
						, (SELECT STUFF((SELECT '/' + airlinePNR
											...
											WHERE (tblBookMaster.riyaPNR = @PNR OR tblBookMaster.riyaPNR = a.riyaPNR) FOR XML PATH ('')) , 1, 1, '')) AS airlinePNR
						...
						, (SELECT STUFF((SELECT '/' + FORMAT(tblBookMaster.arrivalTime, 'dd-MM-yyyy hh:mm:ss tt') FROM tblBookMaster								
												WHERE (tblBookMaster.riyaPNR = @PNR OR tblBookMaster.riyaPNR = a.riyaPNR) FOR XML PATH ('')) , 1, 1, '')) AS arrivaldate
						, tbi.orderId
						, ((SELECT ISNULL(SUM(totalfare + ISNULL(MCOAmount, 0) + ISNULL(ServiceFee, 0) + ISNULL(GST, 0) + ISNULL(BFC, 0) + ISNULL(TotalHupAmount, 0) - ISNULL(TotalEarning, 0)), 0) FROM tblBookMaster(NOLOCK) WHERE riyaPNR = a.RiyaPNR) ) AS totalfare
						, ISNULL(a.ROE, 1) ROE
	...
	BEGIN  
		...
		SELECT * INTO #tempTableA55 FROM  
		 (SELECT  pid, ISNULL(paxfname, '')+ ' ' + paxlname AS paxname  
		   ...
		   , ((SELECT ISNULL(SUM(totalfare + ISNULL(MCOAmount, 0) + ISNULL(BFC, 0) + ISNULL(TotalHupAmount, 0) - ISNULL(TotalEarning, 0)), 0) FROM tblBookMaster(NOLOCK) WHERE riyaPNR = a.RiyaPNR) ) AS totalfare  
		   , ISNULL(a.ROE, 1) ROE   
	  ...
	  BEGIN   
			...
			(SELECT pid, ISNULL(paxfname, '')+ ' ' + paxlname AS paxname  
					...
					, a.deptTime AS deptdate 
					 , a.OrderId  
					 , (a.totalfare + ISNULL(a.MCOAmount, 0) + ISNULL(A.ServiceFee, 0) + ISNULL(A.GST, 0) + ISNULL(a.BFC, 0)+ ISNULL(a.TotalHupAmount, 0) - ISNULL(TotalEarning, 0)) AS totalfare  
					 , ISNULL(a.ROE, 1) ROE  
					 , ISNULL(a.AgentROE, 1) AgentROE  
					 , ISNULL(a.B2BMarkup, 0) B2BMarkup  
					 , ISNULL(a.AgentMarkup, 0) Markup  
    ...
       						, a.RiyaPNR   
							...
							, a.arrivalTime AS arrivaldate  
							, a.OrderId  
							, (a.totalfare + ISNULL(a.MCOAmount, 0) + ISNULL(A.ServiceFee, 0) + ISNULL(A.GST, 0) + ISNULL(a.BFC, 0)+ ISNULL(a.TotalHupAmount, 0) - ISNULL(TotalEarning, 0)) AS totalfar   
							, ISNULL(a.ROE, 1) ROE   
							, ISNULL(a.AgentROE, 1) AgentROE   
							, ISNULL(a.B2BMarkup, 0) B2BMarkup   
							, ISNULL(a.AgentMarkup, 0) Markup   
