...
-- =============================================
CREATE PROCEDURE [dbo].[sp_GetViewPNRDetails_New]
...
BEGIN
	...
	-- START RMS/ATTRIBUTE 
	,(SELECT 
	(CASE WHEN tblPassengerBookDetails.title IS NOT NULL AND tblPassengerBookDetails.title <> '' 
   ...
   , (SELECT 
	 ...
	 , (SELECT  BookMasterItenary.airName AS VendorName
           ...
           ,ISNULL(BookMasterItenary.ParentOrderId, BookMasterItenary.orderid) AS OrderId
			...
			,(CASE WHEN mCountryCurrency.CurrencyCode != @Currency 
					...
					+ CASE WHEN ISNULL(BookMasterItenary.totalFare, 0) != 0 THEN BookMasterItenary.B2BMarkup + BookMasterItenary.BFC + BookMasterItenary.ServiceFee + BookMasterItenary.GST ELSE 0 END) - ISNULL(BookMasterItenary.TotalEarning,0)) AS DECIMAL(10, 2))
		...
		--	, (SELECT (ISNULL(tblPassengerBookDetails.title,'') + '. ' + tblPassengerBookDetails.paxFName + ' ' + ISNULL(tblPassengerBookDetails.paxLName,'') + ' ') AS PaxName
		     ,CASE 
               WHEN tblPassengerBookDetails.paxType = 'LBR' THEN 'ADULT'
            ...
            ,(CASE WHEN mCountryCurrency.CurrencyCode != @Currency 
					THEN CAST((ISNULL(tblPassengerBookDetails.basicFare,0) * tblBookMaster.AgentROE * tblBookMaster.ROE) + 
                   + (CASE WHEN tblBookMaster.B2bFareType IN (1) THEN tblPassengerBookDetails.B2BMarkup ELSE 0 END)
				   + (CASE WHEN ISNULL(tblPassengerBookDetails.MarkOn, '') = 'Markup on Base' THEN tblPassengerBookDetails.BFC ELSE 0 END) AS DECIMAL(10, 2))
			 ...
			 ,(CASE WHEN mCountryCurrency.CurrencyCode != @Currency 
					THEN CAST((ISNULL(tblPassengerBookDetails.totalFare,0) * tblBookMaster.AgentROE * tblBookMaster.ROE) - (ISNULL(tblPassengerBookDetails.PLBCommission, 0)+ISNULL(tblPassengerBookDetails.IATACommission, 0)+ISNULL(tblPassengerBookDetails.DropnetCommission, 0))  AS DECIMAL(10, 2))
					ELSE CAST((ISNULL(tblPassengerBookDetails.totalFare,0) * tblBookMaster.ROE) -(ISNULL(tblPassengerBookDetails.PLBCommission, 0)+ISNULL(tblPassengerBookDetails.IATACommission, 0)+ISNULL(tblPassengerBookDetails.DropnetCommission, 0))  AS DECIMAL(10, 2))
				END) AS TotalFare
              ...
              ,(CASE WHEN mCountryCurrency.CurrencyCode != @Currency 
				 ...
				 +ISNULL(tblPassengerBookDetails.VendorServiceFee,0) AS DECIMAL(10, 2))
					ELSE CAST((ISNULL(tblPassengerBookDetails.totalTax,0) * tblBookMaster.ROE) 
                    + (CASE WHEN tblBookMaster.B2bFareType IN (2, 3) THEN tblPassengerBookDetails.B2BMarkup ELSE 0 END)
					+ (CASE WHEN tblPassengerBookDetails.MarkOn = 'Markup on Tax' THEN tblPassengerBookDetails.BFC ELSE 0 END)
					+ISNULL(tblPassengerBookDetails.VendorServiceFee,0) AS DECIMAL(10, 2))
				END) AS TotalTax
		  ...
		  --,ISNULL(tblPassengerBookDetails.AirlineFee,0) AS AirlineFee
			...
			,(CASE WHEN mCountryCurrency.CurrencyCode != @Currency
					 ...
					 + ISNULL(tblPassengerBookDetails.RFTax, 0)) * tblBookMaster.ROE * tblBookMaster.AgentROE) AS Numeric(18, 2))
		 		...
		 		ELSE CAST(((ISNULL(tblPassengerBookDetails.INTax, 0)
					 ...
					 + ISNULL(tblPassengerBookDetails.WOTax, 0)
					 + ISNULL(tblPassengerBookDetails.OBTax, 0)
					 + ISNULL(tblPassengerBookDetails.RFTax, 0)) *tblBookMaster.ROE) AS Numeric(18, 2))
		 	         END) AS XTTax
			...
			WHERE  BookMasterItenary.riyapnr = @RiyaPNR 
			--tblBookItenary.orderId = @OrderID  or tblBookItenary.ParentOrderId =@OrderID
			ORDER BY tblBookItenary.pkId ASC   
	...
	, (SELECT  Paymentmaster.PKID AS PaymentID
		...
		WHERE    
		Paymentmaster.ParentOrderId LIKE @OrderID + '%' 
    ...
    FROM (
        SELECT 
            ...
            ROW_NUMBER() OVER (PARTITION BY AmendmentRef ORDER BY ID ASC) AS rn
