--exec Sp_GetDetailsforPrint '','ON320P','NA','inr'
create Procedure [dbo].[Sp_GetDetailsforPrintold]
...
where t.orderid=@orderid or t.riyaPNR=@RiyaPNR and  returnFlag =0
...
,c.CurrencyCode
,isnull(tp.BaggageFare,0) BaggageFare
...
where riyaPNR=@RiyaPNR  and (tb.totalFare>0 or (isnull(tb.PreviousRiyaPNR,'')!=''))
group by c.CurrencyCode,tb.MCOAmount,tb.AgentMarkup,tb.ServiceFee,tb.GST,TB.BFC, tb.ReissueCharges,  tp.title, tp.paxFName , tp.paxLName , paxType,tp.BaggageFare
...
--BAGGEGE DETAILS FROM SSR TABLE	
  select distinct ssr.fkPassengerid,tp.baggage,ssr.SSR_Type,SSR.SSR_Name, tp.paxFName +' '+tp.paxLName+' ('+tp.paxType+')' FullName	
...
--MEAL DETAILS FROM SSR TABLE		
 ...
 inner JOIN tblSSRDetails SSR ON SSR.fkpassengerid=tp.pid	
 inner join tblBookMaster tb on tb.pkid=SSR.fkBookMaster	
where  tb.riyaPNR=@RiyaPNR and SSR_Type='Meals' and ssr.SSR_Name is not null
end
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[Sp_GetDetailsforPrintold] TO [rt_read]
    AS [dbo];
