...
CREATE PROCEDURE [dbo].[USP_ERP_Hotel_Activity_serviceFee]      
...
,  case when hbm.ServiceChargesPercent = 0.00 and hbm.TotalServiceCharges>0 then       
...
WHERE HSHVouchred.FkStatusId = 4                                                                            
 ...
 -- )                                                                                                  
and HBM.BookingReference in('TNH00284414')--('TNH00284400')-- ('TNH00284390')---('TNH00284387')--('TNH00283345')                                                                                                  
ORDER BY HBM.pkId DESC                                                                                                  
 ...
 IF @Action = 'HotelCancellation'                                                                        
  ...
  ,'Hotel-Dom' 'ProductType','E-Ticket' as 'TicketType',CheckInDate,CheckOutDate,                                                                         
...
,  case when hbm.ServiceChargesPercent = 0.00 and hbm.TotalServiceCharges>0 then       
 ...
 ,hbm.ServiceCharges 'ServiceFee'      
   ,0 'TaxAmount',MarkupAmount 'MarkupOnBaseFare',                                                                                                  
  ...
  LEFT JOIN agentLogin AL ON  AL.UserID = HBM.RiyaAgentID                                                                        Where Sh.IsActive=1                                                                                                   
  and SH.FkStatusId=7                                                                                                   
...
and HBM.B2BPaymentMode=2                                                                                       
  ...
  and BookingReference in ('TNH00284390'
,'TNH00284400'
,'TNH00284414')--('TNH00284387')--('TNH00284414')  
  Order by HBM.pkId desc                                                                                                  
   END                                                                                             
  ...
  BW.BranchId,BW.SubLed,HB.cityName,HB.Hotel_ERPPushStatus,HB.AgentId,HB.RiyaAgentID          
  ,AL.userTypeID,HB.inserteddate            
   ,HSHVouchred.CreateDate as 'PostingDate' --#1044-PostingDate Should Status Change Date            
              
  ,BW.SubLed,HB.riyaPNR,                                                                                 
 ...
 ,HB.SpecialRemark,HB.TotalChildren,HB.TotalAdults ,Cast(HB.TotalAdults as int) + Cast(HB.TotalChildren as int) as TotalPax,HB.FinalROE,HB.HotelTotalGross,B2BC.SupplierCommission,                                                                            
  ...
  B2BR.CustomerCOde,B2BR.Icast as obcust,B2BR.AgencyName,HB.providerConfirmationNumber,HB.AgentRefNo,HB.OBTCNo,      
 ...
 select * from tblInterBranchWinyatra          
   */                                                                
   -- HSHVouchred.FkStatusId=4 and          
  (                                          
 ...
 AND isnull(HB.winyatraError,'') !='Invalid PNR No. Or PNR No. already converted into Invoice !!!'                             
   and HB.BookingReference in ('TNHAPI00131716')          
 /*  ('TNHAPI00131871',          
'TNHAPI00131867',          
'TNHAPI00131866',          
'TNH00281484',          
...
THEN 'CC1011000'                                                                      
WHEN BR.Country = 'USA'                                                                      
THEN 'CC1010000'                WHEN BR.Country = 'UNITED KINGDOM'                                               
THEN 'CC1013000'                                                                      
ELSE 'CC1001000' END AS DivisionCode                                                                                                  
 ,MU.EmployeeNo 'EmployeeCode'                                                                                                  
 ,HBM.BookingCurrency AS 'SalesCurrencyCode'                                                                                                  
