...
CREATE Procedure [dbo].[Proc_UpdateBookingStatus]                  
...
update Hotel_BookMaster set CurrentStatus=@hstatus,AddCancelCharge=@AddCanCharges,agentCancellationCharges=@CancelCharge,        
SupplierCancellationCharges=@SupplierCharge,AgentRemark=@CancelRemark,CancelledBy=@AgentId,CancellationDate=GETDATE(),ModeOfCancellation='API' where pkId=@BookingPkid         
...
(fkbookid, FieldName, FieldValue, InsertedDate, InsertedBy, UpdatedType)          
 SELECT @BookingPkid, 'Cancelled', 'API', GETDATE(), @MainAgentid, 'CancellationStatus'          
