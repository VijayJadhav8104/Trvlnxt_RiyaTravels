--sp_helptext [USP_ERP_Hotel_Activity]_jan2025] 'SightSeeingCancellation'                        
--=================================                        
...
CREATE PROCEDURE [dbo].[USP_ERP_Hotel_Activity_jan2025]                                                                           
...
and HBM.BookingReference in ('TNH00278117')                                                                          
...
WHERE HSHVouchred.FkStatusId = 4                                                                          
 ...
 --WHEN HBM.providerId in ('rt-vtactivities-live','rt-hbactivities-live') then 'CA'                                                                   --end                                                                          
  and HBM.BookingRefId in ('TNA0001554','TNA0001555')                                                                          
...
ELSE 'CC1001000' END AS DivisionCode                                                                          
 ...
 --,case when BR.Country = MU.CountryID then '0' else '1' end as Intcompanytrans    
 ,SightSeeing_ERPCanPushStatus,SightSeeing_ERPCanResponseID    
  ,HBM.ROEValue,HBM.FinalROE      
FROM SS.SS_BookingMaster HBM                                                                    
LEFT JOIN B2BRegistration BR ON HBM.AgentID = BR.FKUserID                                                                          
LEFT JOIN Paymentmaster PM ON PM.order_id = HBM.bookingrefid                                                                          
