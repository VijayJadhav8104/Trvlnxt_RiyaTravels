CREATE PROCEDURE [dbo].[USP_ERP_Activity] --'CABookingtickets','','WGZVFE-1',''  
...
BEGIN          
 IF @Action = 'Cancellationtickets'  --Marine Cancellation          
  ...
  when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) < 10 and BM.Country = 'CA' then 'CVEND00041'  
     ...
     PB.YRTax, PB.YQ, PB.INTax,(PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax', PB.OCTax, PB.ExtraTax,  PB.JNTax as 'K3Tax',PB.YMTax,  
 ...
 IF @Action = 'MarineBookingtickets'          
   ...
   case when BM.BookingSource = 'Retrive PNR Accounting' THEN PB.FMCommission   
       ...
       THEN PB.Markup  
       ELSE PB.Markup END AS VendIATACommAmount,isnull(PB.VendorServiceFee,'0.00') as VendorServiceFee,  
   ISNULL(BM.ValidatingCarrier,BM.airCode) as ValidatingCarrier, PB.IATACommission as CustIATACommAmount ,(Cast(isnull(PB.PLBCommission,0) as decimal(18,2)) + Cast(isnull(PB.DropnetCommission,0) as decimal(18,2)) + CAST(isnull(PB.GSTonPLB,0) as decimal(18
,2)) + CAST(isnull(PB.tdsonplb ,0) as decimal(18,2))) as PLBCommission,  
   case when AC.type = 'FSC' then BM.GDSPNR else BI.airlinePNR end as PNRNo,  
   case when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) < 10then 'BOMVEND007913'   
 ...
 IF @Action = 'B2CCancellationtickets'          
  ...
  --as MealCharges,  
    ...
    PB.paxFName, PB.paxLName, ISNULL(PB.paxFName,'') +' '+ ISNULL(PB.paxLName,'') as PaxName,           
    PB.managementfees, PB.baggage, case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,        
    PB.YRTax, PB.YQ, PB.INTax, (PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax', PB.OCTax, PB.ExtraTax,PB.CancellationCharge,PB.CancellationPenalty,   
 ...
 IF @Action = 'B2CBookingtickets'          
   ...
   when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'CA' then 'CVEND00041'  
    ...
    BM.VendorName,ISNULL(BM.ValidatingCarrier,BM.airCode) as ValidatingCarrier, isnull(PB.IATACommission,0) as CustIATACommAmount ,(Cast(isnull(PB.PLBCommission,0) as decimal(18,2)) + Cast(isnull(PB.DropnetCommission,0) as decimal(18,2)) + CAST(isnull(PB.
GSTonPLB,0) as decimal(18,2)) + CAST(isnull(PB.tdsonplb ,0) as decimal(18,2))) as PLBCommission,  
    case when BM.VendorName='TravelFusion' then 'RIYAAPITF' else BM.OfficeID end as OfficeID,  
  ...
  --as MealCharges,  
    ...
    case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,PB.YRTax,PB.YQ,PB.INTax,(PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax',PB.OCTax,PB.ExtraTax,PM.CardType,PM.MaskCardNumber as 'CardNumber',PM.mer_amount,PM.billing_address,          
 ...
 Case When ED.CustomerCode='AgentCustID' and BR.Icast !='AgentCustID' then BR.Icast  
  ...
  --  and PB.CancERPResponseID is null and (PB.CancERPPuststatus = 0 or PB.CancERPPuststatus is null)             
    ...
    and AL.userTypeID = 2  
    and BM.VendorName Not in ('STS')
	and bm.riyaPNR='IL41N3'
	--and pb.TicketNumber='WSV9TB-6'
 ...
 case when U.employeeNo in ('023344','023394','730356','730991','731027') then '' 
 else U.EmployeeNo end as UserName, ED.AgentCountry,   
  case when BM.VendorName='TravelFusion' then 'IN' else ED.OwnerCountry END AS OwnerCountry,PB.pid,  
   ...
   BM.depDate,BM.arrivalDate,BM.airCode,BM.Vendor_No,BM.IATACommission,BM.deptTime,BM.arrivalTime,BM.frmSector,BM.toSector,          
   BM.flightNo,BM.travClass,BM.AgentID,BM.Country,BM.fromTerminal,BM.toTerminal,BM.equipment,BM.TotalDiscount,Case when AC.type ='FSC' then BM.GDSPNR  else LEFT(PB.TicketNumber, 6)  end as 'GDSPNR',          
   BM.VendorCommissionPercent,PB.ServiceFee,
   Cast(isnull(PB.GSTonPLB,0) as decimal(18,2)) as GSTonPLB,
   BM.AgentROE,BM.AgentCurrency,PB.B2bMarkup,PB.BFC,  
   case when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) < 10then 'RIYAAPITF'         
     when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'IN' then '14338855'        
     when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'AE' then '86215290'        
     when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'US' then '45828661'        
  ...
  --as MealCharges,  
   ...
   PB.managementfees,left(PB.baggage,29) as baggage,case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,PB.YRTax,PB.YQ,PB.INTax,(PB.JNTax+ isnull(PB.k7tax,0)) as 'JNTax',PB.OCTax,PB.ExtraTax,PB.WOTax,PB.BaggageFare,PM.CardType,          
 ...
 IF @Action = 'RBTBookingTickets'          
   ...
   SELECT TOP 10000   
   case when U.employeeNo in ('023344','023394','730356','730991','731027') then ''
   else U.EmployeeNo end as UserName,  
   --ED.VendorCode,  
   case when BM.BookingSource = 'Retrive PNR Accounting' THEN PB.FMCommission   
       when BM.BookingSource = 'Retrive PNR Accounting-TicketNumber' THEN PB.FMCommission   
       when BM.BookingSource ='GDS' THEN PB.FMCommission  
  ...
  --as MealCharges,  
   ...
   BM.RegistrationNumber,PB.title,PB.TicketNumber,PB.pid,PB.basicFare + isnull(pb.LonServiceFee,0) as basicFare ,
   PB.totalTax,PB.paxType,PB.paxFName,PB.paxLName, Cast(isnull(PB.GSTonPLB,0) as decimal(18,2)) as GSTonPLB,          
   PB.managementfees,substring(replace(PB.baggage,'&','and'),0,30) as baggage,case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,PB.YRTax,PB.YQ,PB.INTax,(PB.JNTax+ isnull(PB.k7tax,0)) as 'JNTax', PB.YMTax,  
   ...
   ,case when AL.UserID = 47129 then ISNULL(ATT.UID,'') 
     when AL.GroupId='23' then isnull(ATT.ConcurID,'') 
	  when AL.GroupId = '32' then isnull(replace(ATT.SBU,'&','and'),'') 
   ...
   when al.GroupId='23' then ISNULL(ATT.CostCenter,'')
    when AL.GroupId = '32' then isnull(replace(ATT.InternalOrder,'&','and'),'')
	when AL.GroupId = '33' then isnull(replace(ATT.PurposeofTravel,'&','and'),'') 
   else  ISNULL(ATT.EmpDimession,'') 
   end  as _str1028,   
   case when AL.UserID = 47129 then ISNULL(replace(ATT.Account,'&','and'),'') 
    when AL.GroupId = '32' then isnull(replace(ATT.EmpDimession,'&','and'),'') 
   else ISNULL(replace(ATT.Travelduration,'&','and'),'') end as _str1054,  
   case when AL.UserID = 47129 then ISNULL(ATT.UID,'')
   when al.GroupId in ('23','33') then ISNULL(replace(ATT.EmpDimession,'&','and'),'')
    when AL.GroupId = '32' then isnull(replace(ATT.PurposeofTravel,'&','and'),'') 
   else ISNULL(ATT.Traveltype,'') end as _str1061 ,  
   case when AL.UserID = 47129 then ISNULL(ATT.TravelerType,'') 
   when AL.GroupId = '32' then isnull(replace(ATT.ServiceRequestID,'&','and'),'') 
   else ISNULL(ATT.Changedcostno,'') end as _str1025,  
    case when AL.GroupId = '32' then isnull(replace(ATT.TripID,'&','and'),'') 
   else ISNULL(ATT.TASreqno,'') end  as _str1022, ISNULL(ATT.Companycodecc,'')  as _str1023, ISNULL(ATT.Projectcode,'') as _str1024,  
    Case when ED.AgentCountry = ED.OwnerCountry then '0' else '1' end as Intcompanytrans  
   FROM tblBookMaster BM WITH (NOLOCK)          
   LEFT JOIN tblPassengerBookDetails PB WITH (NOLOCK) ON BM.pkId = PB.fkBookMaster          
   INNER JOIN Paymentmaster PM WITH (NOLOCK) ON BM.orderId = PM.order_id            
 ...
 IF @Action = 'RBTCancellationTickets'          
   BEGIN          
	...
	(Cast(isnull(PB.PLBCommission,0) as decimal(18,2)) 
	+ Cast(isnull(PB.DropnetCommission,0) as decimal(18,2))) as PLBCommission, 
	'0' as Surcharge,  Cast(isnull(PB.GSTonPLB,0) as decimal(18,2)) as GSTonPLB, 
  ...
  --as MealCharges,  
    ...
    PB.paxFName, PB.paxLName, ISNULL(PB.paxFName,'') +' '+ ISNULL(PB.paxLName,'') as PaxName,           
    PB.managementfees, PB.baggage, case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,  
    PB.YRTax, PB.YQ, PB.INTax, (PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax', PB.OCTax, PB.ExtraTax,PB.YMTax,  
    PM.CardType, PM.MaskCardNumber as 'CardNumber', PM.mer_amount, PM.billing_address, PM.billing_name, PM.order_id, PM.tracking_id,          
    --BI.airlinePNR, BI.cabin, BI.farebasis,          
    ATT.VesselName, ATT.TR_POName, ATT.Bookedby, ATT.AType, ATT.RankNo,  
    Case When ED.CustomerCode='AgentCustID' and BR.Icast !='AgentCustID' then BR.Icast  
 ...
 IF @Action = 'RBT-US-CABookingTickets'          
  ...
  PB.basicFare + isnull(PB.Markup,0) + isnull(PB.LonServiceFee,0) as basicFare, PB.totalTax, PB.JNTax as 'K3Tax', PB.YRTax, PB.YQ, PB.INTax, (PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax', PB.OCTax, PB.ExtraTax, PB.RFTax, PB.WOTax,  PB.YMTax,  
  ...
  when AL.GroupId in ('10','34') then Cast(ISNULL(ATT.TravelRequestNumber,'0') as varchar(50))  
 ...
 when AL.GroupId = '32' then isnull(replace(ATT.SBU,'&','and'),'')
  when AL.GroupId = '35' then isnull(replace(ATT.Department,'&','and'),'') 
   ...
   when Al.GroupId='20' then ISNULL(REPLACE(ATT.CostCentre,'&','and'),'')  
    when Al.GroupId='32' then ISNULL(REPLACE(ATT.PurposeofTravel,'&','and'),'') 
	when Al.GroupId='34' then ISNULL(REPLACE(ATT.ApproverName,'&','and'),'') 
	when Al.GroupId='35' then ISNULL(REPLACE(ATT.[Location],'&','and'),'') 
  ...
  when (BR.Icast in ('UCUST00654','UCUST00615','BCUST00026','UCUST00615A','UCUST00615B','BCUST00026A','CUST001580','CUST001581','CUST001582','CUST001605','CUST001606') or   
   ...
   when Al.GroupId = '32' then isnull(replace(ATT.InternalOrder,'&','and'),'') 
   when Al.GroupId = '35' then isnull(replace(ATT.PurposeofTravel,'&','and'),'') 
  else ISNULL(replace(ATT.EmpDimession,'&','and'),'') end as _str1028,   
  case   
  when AL.GroupId = '18' then ISNULL(replace(ATT.DEVIATIONAPPROVER,'&','and'),'')   
  ...
  when AL.GroupId = '32' then ISNULL(replace(ATT.TripID,'&','and'),'') 
  ...
  case when AL.GroupId = '18' then
    ISNULL(Replace(ATT.TypeofTransaction,'&','&amp;'),'')
  else
  ISNULL(ATT.Companycodecc,'') end as _str1023,  
 ...
 -- and BM.riyaPNR in ('1JR93Z')--,'2MB189','E39I59','V134RR','ID1W22','6M0EO2')  
  ORDER BY BM.IssueDate DESC  
  END  
 ...
 IF @Action = 'RBT-US-CACancellationTickets'          
  ...
  --as MealCharges,  
    ...
    PB.paxFName, PB.paxLName, ISNULL(PB.paxFName,'') +' '+ ISNULL(PB.paxLName,'') as PaxName,           
    PB.managementfees, PB.baggage, case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,  
    PB.YRTax, PB.YQ, PB.INTax, (PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax', PB.OCTax, PB.ExtraTax, PB.WOTax , PB.YMTax,  
    PM.CardType, PM.MaskCardNumber as 'CardNumber', PM.mer_amount, PM.billing_address, PM.billing_name, PM.order_id, PM.tracking_id,          
    --BI.airlinePNR, BI.cabin, BI.farebasis,          
    ATT.VesselName, ATT.TR_POName, ATT.Bookedby, ATT.AType, ATT.RankNo,  
    Case When ED.CustomerCode='AgentCustID' and BR.Icast !='AgentCustID' then BR.Icast  
  ...
  case when AL.GroupId = '3' then ISNULL(ATT.FareRule,'') else '' end as _str2031,  
  case when AL.GroupId = '3' then ISNULL(ATT.FareType,'') else '' end as _str2030,  
  ISNULL(ATT.Travelduration,'')  as _str1054, ISNULL(ATT.TASreqno,'')  as _str1022, ISNULL(ATT.Companycodecc,'')  as _str1023,
  REPLACE(ISNULL(ATT.Projectcode, ''), '&', 'and')  as _str1024,  
  ATT.LOWEST_LOGICAL_FARE_1 as _str1008, ATT.LOWEST_LOGICAL_FARE_2 as _str1010, ATT.LOWEST_LOGICAL_FARE_3 as _str1018, ATT.EmpDimession as _str1005,  
  Case when ED.AgentCountry = ED.OwnerCountry then '0' else '1' end as Intcompanytrans  
    FROM tblBookMaster BM WITH (NOLOCK)  
    LEFT JOIN tblPassengerBookDetails PB WITH (NOLOCK) ON BM.pkId = PB.fkBookMaster          
    ...
    and AL.userTypeID = 5  
    and BM.totalFare > 0  
 --   and BM.Riyapnr not in ('TRN1I1W73L')  
    and BM.VendorName Not in ('STS')  
    ORDER BY BM.IssueDate DESC  
   END          
 ...
 IF @Action = 'RBT_MCO_Bookingtickets'   
   BEGIN          
    ...
    case when CD.Configuration = 'RiyaCard' THEN 'CORPORATE' when CD.Configuration = 'CustomerCard' THEN 'CUSTOMER' when PM.payment_mode = 'passThrough' THEN 'CUSTOMER' else '' END as UATP,  
    PB.MCOAmount,BM.MCOAmount,ED.AgentCountry,ED.OwnerCountry,ED.ERPCountry,BM.MainAgentId,U.UserName,BM.bookingStatus,(Cast(isnull(PB.PLBCommission,0) as decimal(18,2)) + Cast(isnull(PB.DropnetCommission,0) as decimal(18,2)) + CAST(isnull(PB.GSTonPLB,0) 
as decimal(18,2)) + CAST(isnull(PB.tdsonplb ,0) as decimal(18,2))) as PLBCommission,PB.IATACommission as CustIATACommAmount ,  
    --ED.VendorCode,  
  --   (select sum(SSR_Amount)  from tblSSRDetails s  
  --where fkPassengerid in (select pid from tblPassengerBookDetails where  paxFName+isnull(paxLName,'')  
  --in (select paxFName+isnull(paxLName,'') from tblPassengerBookDetails where pid=PB.pid))  
  ...
  Else ED.VendorCode End as VendorCode,  
   ...
   --Else 'CASH' End As UATP,  
    ...
    PB.baggage, case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,BM.AgentMarkup,PB.JNTax as 'K3Tax',PB.YRTax, PB.YQ, PB.INTax, (PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax',PB.OCTax,  
 ...
 IF @Action = 'RBT_MCO_Cancellationtickets'          
  ...
  BM.frmSector,BM.toSector,BM.flightNo,BM.travClass,BM.AgentID,BM.Country,BM.fromTerminal,BM.toTerminal,BM.equipment,(Cast(isnull(PB.PLBCommission,0) as decimal(18,2)) + Cast(isnull(PB.DropnetCommission,0) as decimal(18,2)) + CAST(isnull(PB.GSTonPLB,0) as
 decimal(18,2)) + CAST(isnull(PB.tdsonplb ,0) as decimal(18,2))) as PLBCommission,  
 ...
 IF @Action = 'HolidayCancellationtTickets'  
   ...
   case when CD.Configuration = 'RiyaCard' THEN 'CORPORATE' when CD.Configuration = 'CustomerCard' THEN 'CUSTOMER' when PM.payment_mode = 'passThrough' THEN 'CUSTOMER' else '' END as UATP,  
    ...
    PB.baggage, case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,PB.YRTax, PB.YQ, PB.INTax,(PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax', PB.OCTax, PB.ExtraTax,PM.CardType, PM.MaskCardNumber as 'CardNumber', PM.mer_amount, PM.billing_address,  
 ...
 IF @Action = 'HolidayBookingTickets'          
  BEGIN          
   SELECT TOP 10000
   case when U.employeeNo in ('023344','023394','730356','730991','731027') then '' else 
   U.EmployeeNo end as UserName,
...
B2BR.BranchCode
   ...
   case when BM.BookingSource = 'Retrive PNR Accounting' THEN PB.FMCommission   
       ...
       when (BM.BookingSource ='Web' and PB.Markup >0)  
       THEN PB.Markup  
       ELSE PB.Markup END AS VendIATACommAmount,BM.totalFare,
	   BM.orderId,isnull(PB.VendorServiceFee,'0.00') as VendorServiceFee,  
   ...
   PB.managementfees,PB.baggage,case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,PB.YRTax,PB.YQ,PB.INTax,(PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax',PB.OCTax,PB.ExtraTax,PB.BaggageFare,PM.CardType,          
   PM.MaskCardNumber as 'CardNumber',PM.mer_amount,PM.billing_address,PM.billing_name,PM.order_id,PM.tracking_id,--BI.airlinePNR,BI.cabin,BI.farebasis,          
...
ON ( 
  ...
  IF @Action = 'WinYatra-HolidayBookingTickets'          
   ...
   BM.pkId,BM.riyaPNR,BM.inserteddate,BM.IssueDate,BM.CounterCloseTime,ISNULL(BM.ValidatingCarrier,BM.airCode) as ValidatingCarrier,
   PB.IATACommission as CustIATACommAmount ,
   (Cast(isnull(PB.PLBCommission,0) as decimal(18,2)) 
   + Cast(isnull(PB.DropnetCommission,0) as decimal(18,2))) as PLBCommission,
   Cast(isnull(PB.GSTonPLB,0) as decimal(18,2)) as GSTonPLB,
  ...
  as MealCharges,  
   ...
   Icast= 'BOMCUST002120A',  
   LocationCode = 'BOM',   
   Code = 'BR2002000',  PR.EmpCode,case when BM.VendorName = 'Amadeus' then 'FSC' else AC.type end as 'AirLineType',
   	(Select top 1 ISNULL(ma.OBTCno,'') AS OBTCno from mAttrributesDetails ma Where ma.OrderID=bm.orderId AND ISNULL(ma.OBTCno,'')!='') AS 'OBTCno', 
   (Select SUBSTRING((  
   ...
   FROM tblBookMaster BM WITH (NOLOCK)  
    ...
    LEFT JOIN tblOwnerCurrency CU WITH (NOLOCK) on BM.OfficeID = CU.OfficeID           
    LEFT JOIN AirlineCode_Console AC WITH (NOLOCK) ON AC.Airlinecode = BM.airCode          
    LEFT Join mCardDetails CD WITH (NOLOCK) ON CD.CardNumber = PM.EnCardNumber AND CD.Status=1 
    LEFT Join B2BMakepaymentCommission BBM WITH (NOLOCK) on BBM.orderId = BM.OrderId  
    WHERE BM.IsBooked =1 and BM.totalFare > 0 and BM.AgentID != 'B2C'       
    and (BM.Isfakebooking = 0 or BM.Isfakebooking is null)      
	and BM.BookingStatus In (1,6) --= 4 For Cancellation  =1 For Booking  --As confirm with Mansavee          
  ...
  as MealCharges,  
   ...
   -- and PB.ERPResponseID is not null and (PB.ERPPuststatus != 0 or PB.ERPPuststatus is not null)          
    ...
    and ED.AgentCountry = BM.Country and ED.ERPCountry = BM.Country  
    and BM.totalFare > 0  
   -- and BM.riyaPNR in ('TRN1Y5ARM7','TRNJC102RD','TRND5D7YP9')  
   -- and (ATT.OBTCno != '' or ATT.OBTCno != null)  
    and ((AL.userTypeID = 2 and B2BR.Icast in (select Icust from tblInterBranchWinyatra))  
    or AL.userTypeID IN(4))--For Holiday usertype is 4 ***     
 ...
 Begin
	...
	when U.employeeNo in ('023344','023394','730356','730991','731027') then ''
 else U.EmployeeNo end as UserName, 
   ...
   case when BM.VendorName='TravelFusion' then 'INR' else Cu.Currency end  as 'Currency',BM.ROE,          
   BM.RegistrationNumber,PB.title,PB.TicketNumber,PB.pid,PB.basicFare + isnull(PB.LonServiceFee,0) as basicFare,PB.totalTax,
  case when PB.paxType='LBR' then 'ADULT'
   ...
   PB.WOTax,PB.INTax,(PB.JNTax) as 'JNTax',isnull(PB.k7tax,0) as 'K7Tax',PB.OCTax,PB.ExtraTax,PB.BaggageFare,PM.CardType, PB.YMTax,  
   PM.MaskCardNumber as 'CardNumber',PM.mer_amount,PM.billing_address,PM.billing_name,PM.order_id,PM.tracking_id,--BI.airlinePNR,BI.cabin,BI.farebasis,          
  ...
  Replace(ATT.TR_POName, '&','and') end as 'TR_POName',
  case when Al.GroupId='32' then  ISNULL(REPLACE(ATT.ServiceRequestID,'&','and'),'') 
  else ATT.Bookedby end as Bookedby,
  ATT.AType,  
  case when AL.GroupId='16' then  ISNULL(replace(ATT.EmpDimession,'&','and'),'')  
  when AL.GroupId='32' then  ISNULL(replace(ATT.PurposeofTravel,'&','and'),'')  
  else  
  ATT.RankNo end as 'RankNo', 
  case when AL.GroupId='32' then ISNULL(replace(ATT.EmpDimession,'&','and'),'')
  else '' end as str1054, 
  case when AL.GroupId='32' then  ISNULL(replace(ATT.TripID,'&','and'),'')
  else '' end as str1022,
 ...
 Begin   
  SELECT TOP 10000 PB.ticketnumber,U.EmployeeNo as UserName, BM.bookingStatus,  
   ...
   when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'CA' then 'CVEND00041'  
    ...
    PB.paxFName, PB.paxLName, ISNULL(PB.paxFName,'') +' '+ ISNULL(PB.paxLName,'') as PaxName,           
    PB.managementfees, PB.baggage, case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,           
    PB.YRTax, PB.YQ, PB.INTax,(PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax', PB.OCTax, PB.ExtraTax,          
    PM.CardType, PM.MaskCardNumber as 'CardNumber', PM.mer_amount, PM.billing_address, PM.billing_name, PM.order_id, PM.tracking_id,          
    --BI.airlinePNR, BI.cabin, BI.farebasis,          
    ATT.VesselName, ATT.TR_POName, ATT.Bookedby, ATT.AType, ATT.RankNo,  
...
-- '180409' AS UserName,
  ...
  PB.title, PB.TicketNumber, PB.basicFare + isnull(PB.Markup,0) + isnull(PB.LonServiceFee,0) as basicFare, PB.totalTax, PB.paxType, PB.JNTax as 'K3Tax',          
    ...
    PB.YRTax, PB.YQ, PB.INTax,(PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax', PB.OCTax, PB.ExtraTax, PB.RFTax, PB.WOTax,  PB.YMTax,       
    CASE 
    WHEN LOWER(PM.CardType) = 'mastercard' THEN 'MASTER'
    ELSE PM.CardType
END AS CardType,case when PM1.ParentOrderId is not null then PM1.mer_amount else PM.mer_amount end as 'mer_amount', PM.billing_address, PM.billing_name, PM.order_id, PM.tracking_id,  
    --BI.airlinePNR, BI.cabin, BI.farebasis,  
    PB.ServiceFee as 'ServiceFees',  
  --    (select sum(SSR_Amount)  from tblSSRDetails s  
  --where fkPassengerid in (select pid from tblPassengerBookDetails where  paxFName+isnull(paxLName,'')  
 ...
 IF @Action = 'US-CACancellationtickets'          
  ...
  --as MealCharges,  
    ...
    PB.paxFName, PB.paxLName, ISNULL(PB.paxFName,'') +' '+ ISNULL(PB.paxLName,'') as PaxName,           
    PB.managementfees, PB.baggage, case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,          
    PB.YRTax, PB.YQ, PB.INTax,(PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax', PB.OCTax, PB.ExtraTax,  PB.WOTax,        
    PM.CardType, PM.MaskCardNumber as 'CardNumber', PM.mer_amount, PM.billing_address, PM.billing_name, PM.order_id, PM.tracking_id,          
    --BI.airlinePNR, BI.cabin, BI.farebasis,          
    ATT.VesselName, ATT.TR_POName, ATT.Bookedby, ATT.AType, ATT.RankNo,          
 ...
 IF @Action = 'US_CAD_MCO_Bookingtickets'   
  ...
  --as MealCharges,  
   ...
   case when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) < 10then 'BOMVEND007913'   
   when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'IN' then 'VEND000178'  
   when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'AE' then 'VEND00102'     when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'US' then 'UVEND00066'  
   when BM.VendorName='TravelFusion' and LEN(PB.ticketNum) > 10and BM.Country = 'CA' then 'CVEND00041'  
   else ED.VendorCode end as VendorCode,  
   -- Case When PM.payment_mode='passThrough' then 'CUSTOMER'           
   --When PM.payment_mode='Credit' then 'CORPORATE'           
   --Else 'CASH' End As UATP,  
    ...
    BM.totalFare,PB.MCOAmount,  
    cast((Isnull(PB.MCOAmount,0)-Isnull(PB.MCOAmount,0)*3.5/100)as decimal(18,2)) as McoCommissionAmount,PB.managementfees,  
    PB.baggage, case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup,BM.AgentMarkup,PB.JNTax as 'K3Tax',PB.YRTax, PB.YQ, PB.INTax,
	(PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax',PB.OCTax,  
    PB.ExtraTax,PB.RFTax,PM.CardType, PM.MaskCardNumber as 'CardNumber', PM.mer_amount, PM.billing_address, PM.billing_name, PM.order_id, PM.tracking_id,  
    --BI.airlinePNR, BI.cabin, BI.farebasis,          
    PB.ServiceFee as 'ServiceFees', ISNULL(PB.B2bMarkup,0) +ISNULL(PB.BFC,0) as 'MarkupOnBaseFare' ,'' as MarkupOnTaxFare , '' as Narration,          
    --BM.AgentROE as 'SalesExchangeRate',BM.AgentCurrency as 'SalesCurrencyCode',          
 ...
 IF @Action = 'US_CAD_MCO_Cancellationtickets'          
  ...
  Where BM.IsBooked =1 and BM.totalFare > 0 and BM.AgentID != 'B2C'  
   ...
   BEGIN          
      SELECT TOP 10000 
	  Case when ssr.EMDTicketNumber is null then SSR.ERPTicketNum
	  else SSR.EMDTicketNumber end as ERPTicketNum,
	  	case when ssr.EMDTicketNumber is not null then 'EMD'
	else 'SSR' end as TicketType,
	  ED.AgentCountry,ED.OwnerCountry, ED.ERPCountry, BM.MainAgentId,SSR.ParentOrderId,SSR.ERPPushStatus      
...
CASE 
    WHEN PM.cardconfigurationType IS NULL THEN 
        CASE 
            WHEN CD.Configuration = 'RiyaCard' THEN 'CORPORATE'
            WHEN CD.Configuration = 'CustomerCard' THEN 'CUSTOMER' 
            WHEN PM.payment_mode = 'passThrough' THEN 'CUSTOMER' 
            ELSE '' 
        END
    ELSE 
        CASE 
            WHEN PM.CardConfigurationType = 'Riya' THEN 'CORPORATE'
            WHEN PM.CardConfigurationType = 'Customer' THEN 'CUSTOMER' 
            ELSE '' 
        END
END AS UATP,
    ...
    CASE 
    WHEN al.userTypeID = 4 THEN
        CASE 
            WHEN BM.OfficeID = 'DFW1S212A' THEN 'UCUST00505'   
            WHEN BM.OfficeID = 'YWGC4211G' THEN 'CCUST00004'   
            WHEN BM.OfficeID = 'DXBAD3359' THEN 'CUST000324'   
            ELSE IBM.Icust   
        END
    ELSE
        ISNULL(BR.Icast, BR1.Icast) 
END AS Icast, BM.riyaPNR, BM.inserteddate, BM.CounterCloseTime,          
    BM.RegistrationNumber, BM.IssueDate,BM.depDate, BM.arrivalDate, BM.airCode, BM.Vendor_No, BM.IATACommission, BM.deptTime,          
    BM.arrivalTime, BM.frmSector, BM.toSector, BM.flightNo, BM.travClass,          
    BM.totalFare, BM.AgentID, BM.Country, BM.fromTerminal, BM.toTerminal, BM.equipment,          
    BM.TotalDiscount, Case when AC.type ='FSC' then BM.GDSPNR  else LEFT(PB.TicketNumber, 6)  end as 'GDSPNR', BM.VendorCommissionPercent,   
    case when BM.VendorName='TravelFusion' then 'RIYAAPITF' else BM.OfficeID end as OfficeID, ED.OwnerID,ED.ERPCountry, BM.orderId,          
  PB.title, 
  	  Case when ssr.EMDTicketNumber is null then PB.TicketNumber
	  else SSR.EMDTicketNumber end as TicketNumber,  
  PB.basicFare + isnull(PB.Markup,0) + isnull(PB.LonServiceFee,0) as basicFare, PB.totalTax, PB.paxType, PB.JNTax as 'K3Tax',          
    PB.paxFName, PB.paxLName, ISNULL(PB.paxFName,'') +' '+ ISNULL(PB.paxLName,'') as PaxName,           
    PB.managementfees, PB.baggage,case when BM.BookingSource!='ManualTicketing' then PB.Markup else 0 end as Markup, BM.AgentMarkup,          
    PB.YRTax, PB.YQ, PB.INTax, (PB.JNTax + isnull(PB.k7tax,0)) as 'JNTax', PB.OCTax, PB.ExtraTax, PB.RFTax, PB.WOTax,  PB.YMTax,       
 ...
 (cast((isnull((CASE WHEN BM.B2bFareType=1 THEN  (PB.B2BMarkup/isnull(BM.AgentROE,0)) ELSE 0 END)* BM.ROE,0))as decimal(18,2)) +PB.BFC) as MarkupOnFare_          
    ...
    LEFT JOIN mAttrributesDetails ATT WITH (NOLOCK) ON PB.pid = ATT.fkPassengerid  
    LEFT JOIN tblERPDetails ED WITH (NOLOCK) ON BM.OfficeID = ED.OwnerID and BM.Country=ED.AgentCountry         
    LEFT JOIN B2BRegistration BR WITH (NOLOCK) ON BM.AgentID = BR.FKUserID and AL.ParentAgentID is null          
    LEFT JOIN B2BRegistration BR1 WITH (NOLOCK) ON AL.ParentAgentID = BR1.FKUserID and AL.ParentAgentID is not null          
    LEFT JOIN AirlineCode_Console AC WITH (NOLOCK) ON AC.Airlinecode = BM.airCode          
    LEFT JOIN tblOwnerCurrency TOC WITH (NOLOCK) ON TOC.OfficeID = BM.OfficeID and BM.VendorName=TOC.Vendor         
    LEFT JOIN AirlineMaster AM WITH (NOLOCK) ON AM.Airlinecode = BM.airCode          
    LEFT Join mCardDetails CD WITH (NOLOCK) ON CD.CardNumber = PM.EnCardNumber  
    --LEFT JOIN tblBookItenary BI ON BM.orderId = BM.orderId          
    LEFT Join B2BMakepaymentCommission BBM WITH (NOLOCK) on BBM.orderId = BM.OrderId  
	...
	LEFT JOIN tblInterBranchWinyatra IBM WITH (NOLOCK)
ON ( 
    (BR.StateID = IBM.fkStateId AND IBM.Country = 'IN') 
    OR 
    (BR.StateID IS NULL AND BR1.StateID = IBM.fkStateId AND IBM.Country = 'IN')
)  
    WHERE  
    BM.IsBooked =1 
	and BM.AgentID != 'B2C'      
    ...
    and BM.inserteddate > (Getdate()-365) -- '2021-07-31 23:59:59.999'  
    and SSR.SSR_Amount > 0 
	and ed.ERPCountry=bm.country
--	and BM.riyaPNR in  ('TRNH15ZDS7')
	    AND (
        SSR.EMDTicketNumber IS NOT NULL
        OR (
            SSR.EMDTicketNumber IS NULL
			and SSR.ERPTicketNum is not null
            AND PB.ERPResponseID IS NOT NULL
            AND (PB.ERPPuststatus != 0 OR PB.ERPPuststatus IS NOT NULL)
            AND BM.totalFare > 0
        )
    )
    ORDER BY BM.IssueDate DESC      
   ...
   BEGIN          
      SELECT TOP 10000  Case when ssr.EMDTicketNumber is null then SSR.ERPTicketNum
	  else SSR.EMDTicketNumber end as ERPTicketNum, 
	 case when ssr.EMDTicketNumber is not null then 'EMD'
	else 'SSR' end as TicketType,
	  ED.AgentCountry,ED.OwnerCountry, ED.ERPCountry, BM.MainAgentId,SSR.ParentOrderId,SSR.ERPPushStatus      
	...
	and ed.ERPCountry=bm.country
	  AND (
        ...
        OR (
            SSR.EMDTicketNumber IS NULL
			and SSR.ERPTicketNum is not null
            AND PB.CancERPResponseID IS NOT NULL
            AND (PB.CancERPPuststatus != 0 OR PB.CancERPPuststatus IS NOT NULL)
            AND BM.totalFare > 0
        )
    )
    --and BM.riyaPNR = '244SH3'  
...
WHERE ParentOrderId = @orderId
  AND fkPassengerid = @tblPassengerid
  AND SSR_Amount > 0
  AND (
        (EMDTicketNumber IS NOT NULL AND EMDTicketNumber = @erpTicketNum)
        OR
        (EMDTicketNumber IS NULL AND ERPTicketNum = @erpTicketNum)
      )
  AND (ERPResponseID = '' OR ERPResponseID IS NULL)
  AND (ERPPushStatus = '' OR ERPPushStatus IS NULL);

  --SELECT * from tblSSRDetails where ParentOrderId = @orderId   
  --and fkPassengerid = @tblPassengerid and erpTicketNum=@erpTicketNum and (ERPResponseID = '' or ERPResponseID is null) and (ERPPushStatus = '' or ERPPushStatus is null)  
   END  
   if(@flag = 'Book1') 
   BEGIN
      SELECT SSR_Type,
EMDTicketnumber,'' as ERPTicketNum,
    SUM(SSR_Amount) AS SSR_Amount
FROM tblSSRDetails
WHERE 
--ParentOrderId = @orderId AND
   SSR_Amount > 0
  AND EMDTicketNumber = @erpTicketNum
   AND (ERPResponseID = '' OR ERPResponseID IS NULL)
  AND (ERPPushStatus = '' OR ERPPushStatus IS NULL)
  group by SSR_Amount,SSR_Type,
EMDTicketnumber
UNION ALL
SELECT 
SSR_Type,'' as EMDTicketnumber,
ERPTicketNum,
 SUM(SSR_Amount) AS SSR_Amount
FROM tblSSRDetails
WHERE ParentOrderId = @orderId
  AND SSR_Amount > 0
  AND EMDTicketNumber IS NULL 
  AND ERPTicketNum = @erpTicketNum
 -- AND fkPassengerid = @tblPassengerid
  AND (ERPResponseID = '' OR ERPResponseID IS NULL)
  AND (ERPPushStatus = '' OR ERPPushStatus IS NULL)
  group by SSR_Amount,SSR_Type,
ERPTicketNum
   ...
   BEGIN  
  SELECT * from tblSSRDetails where ParentOrderId = @orderId   
  and fkPassengerid = @tblPassengerid and erpTicketNum=@erpTicketNum--and (CancERPResponseID = '' or CancERPResponseID is null) and (CancERPPuststatus = '' or CancERPPuststatus is null)  
   END  
   END  
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[USP_ERP_Activity] TO [rt_read]
    AS [dbo];
