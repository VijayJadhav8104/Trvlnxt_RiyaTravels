...
BEGIN
	...
	IF(EXISTS(SELECT 1 FROM mUser where  UserName=@userID ))--and  [Password]=@passwd
		BEGIN
			SELECT TOP 1 U.Id,U.FullName, M.Path,U.IsResetPassword, U.RoleID,UserTypeId
			From mUser U
			JOIN  mRoleMapping R ON R.RoleID = U.RoleID
			JOIN mMenu M ON R.MenuID = M.ID
			where  U.UserName=@userID  AND R.IsActive = 1 AND M.Path is not null and m.Module='Console'  AND M.MenuName <> 'Reports'
			...
			--SET SessionID=@SessionID WHERE   UserName=@userID and  [Password]=@passwd
			 ...
			 if(@Flag='Console')
			begin 
			UPDATE mUser SET ConsoleSessionID=@SessionID WHERE   UserName=@userID --and  [Password]=@passwd  
			end
			else
			begin
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[Sp_UserLogin] TO [rt_read]
    AS [dbo];
