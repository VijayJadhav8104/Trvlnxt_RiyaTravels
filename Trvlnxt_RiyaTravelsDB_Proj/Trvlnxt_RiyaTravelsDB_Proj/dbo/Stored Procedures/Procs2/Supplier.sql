...
@VirtualBalance,                              
 ...
 IsActive=@ActionStatus,            
   ...
   IsHotelContentAvail=@IsHotelContentAvail    
...
,HSM.RateDisplay as 'RateDisplay'     
,case when HSM.SupplierCurrency=-1 then 'MULTI' else MC.Value                                       
 end  as 'Currency'     
 ,MU.UserName as 'Usernames'                                            
 ,[SupplierCharges]    
 ,Isnull(IsHotelContentAvail,0) as IsHotelContentAvail    
...
,MC.Value as 'Currency'                                        
 ...
 if(@Action='Delete')                                      
 begin                                      
  update B2BHotelSupplierMaster set ModifiedBy=@ModifiedBy,ModifiedOn=GETDATE(),      
  IsDelete=1,      
  SupplierName='old'+(select SupplierName from B2BHotelSupplierMaster where Id=@Id)        
  ,RhSupplierId='old'+(select RhSupplierId from B2BHotelSupplierMaster where Id=@Id)      
  where Id=@Id                
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[Supplier] TO [rt_read]
    AS [dbo];
