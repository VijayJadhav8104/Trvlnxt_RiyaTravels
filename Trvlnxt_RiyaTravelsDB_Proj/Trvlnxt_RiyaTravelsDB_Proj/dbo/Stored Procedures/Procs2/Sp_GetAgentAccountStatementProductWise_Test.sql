...
--exec Sp_GetAgentAccountStatementProductWise_Test '2024-04-15','2024-04-15','','','4','46999','IN','Hotel',''                  
CREATE PROCEDURE [dbo].[Sp_GetAgentAccountStatementProductWise_Test]                  
 @FROMDate Date=NULL                   
 , @ToDate Date=NULL                  
...
--   ,case When B.BookingCountry='IN' then 'Domestic' else 'International' End as 'TravelType'              
 ...
 --  WHEN tAB.TransactionType is not null and SH.FkStatusId=11 THEN convert(VARCHAR(50), ISNULL(TAB.TranscationAmount, '0'))             
  ...
  , CASE WHEN TAB.TransactionType = 'Debit' and TL.BookingStatus='Vouchered' THEN convert(VARCHAR(50), ISNULL(TAB.TranscationAmount, '0'))             
  --WHEN TAB.TransactionType is not null and SH.FkStatusId=11 THEN convert(VARCHAR(50), ISNULL(TAB.TranscationAmount, '0'))             
  ...
  , TL.BookingStatus AS 'Booking Status'               
 ...
 WHERE (@ProductType='ALL' OR @ProductType='Hotel') and (CreditAmount not in ('0') or DebitAmount not in ('0') or [Booking Status]='Failed') order by [DateTime]                       
