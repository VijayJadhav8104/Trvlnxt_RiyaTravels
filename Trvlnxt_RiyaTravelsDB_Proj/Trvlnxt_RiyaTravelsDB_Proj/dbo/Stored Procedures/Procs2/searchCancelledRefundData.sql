...
CREATE PROCEDURE [dbo].[searchCancelledRefundData] --'1441U1Q',1,9
...
--  where PB.IsRefunded=0 and BM.IsBooked=1 and  BM.GDSPNR = @Search and  PB.Iscancelled=0  
--				  and PB.isProcessRefund=1)p ORDER BY 
...
IF OBJECT_ID ( 'tempdb..#tempTableA') IS NOT NULL
  ...
  ( pkId INT, orderId VARCHAR(100),riyaPNR VARCHAR(100), email VARCHAR(100),
    ...
    airName VARCHAR(100), Cancellation_Date DATETIME, sector VARCHAR(500), isProcessRefund VARCHAR(100),Country varchar(2),OfficeID varchar(50),
	AgencyName VARCHAR(50),FailedFlag int
   )
...
SELECT *
 FROM
	( SELECT DISTINCT (BM.pkId), BI.orderId, BM.riyaPNR, BM.emailId as email,  
			 ...
			 ,ISNULL((SELECT B.AgencyName FROM B2BRegistration B WHERE CONVERT(varchar(50),B.FKUserID)=BM.AgentID),'B2C') as AgencyName,FailedFlag
      FROM tblBookMaster BM 
      ...
      WHERE PB.IsRefunded=0 and BM.IsBooked=1 and  BM.GDSPNR = @Search and  PB.Iscancelled=0   and PB.isProcessRefund=1
...
SELECT orderId, riyaPNR,email, mob, min(depDate) as depDate, GDSPNR,airlinePNR, airName,Cancellation_Date,sector,isProcessRefund,Country,OfficeID,AgencyName,FailedFlag
FROM #tempTableA
Group by orderId, riyaPNR,email, mob, GDSPNR,airlinePNR, airName,Cancellation_Date,sector,isProcessRefund,Country,OfficeID,AgencyName,FailedFlag 
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[searchCancelledRefundData] TO [rt_read]
    AS [dbo];
