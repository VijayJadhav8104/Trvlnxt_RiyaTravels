...
--exec USP_ERP_Hotel_Activity_071024  'HotelCancellation','','',''           
CREATE PROCEDURE [dbo].[USP_ERP_Hotel_Activity_071024]                                       
...
BEGIN                                      
 ...
 ,'' 'ChildRate'                                      
 ,'' 'ExtraBed'              
 , HBM.Hotel_ERPResponceID            
 , HBM.Hotel_ERPPushStatus            
...
--HBM.BookingReference in ('TNH00267287' ) and --('TNH00267050','TNH00267057')              
 ...
 -- )         
and HBM.BookingReference in ('TNH00270542')                                                          
...
ORDER BY HBM.pkId DESC         
 ...
 SUBSTRING( HBM.SupplierName ,1,10) as 'SupplierCode',BR.CustomerCOde 'ShipToCustomer',                 
  case  when HBM.BookingCountry = 'AE' then 'VEND00137'         
  WHEN HBM.BookingCountry = 'SA' THEN 'VEND00024'         
   WHEN HBM.BookingCountry = 'US'                                              
   THEN 'UVEND01021'          
   WHEN HBM.BookingCountry = 'CA'                                              
   THEN 'CVEND00242'                   
  ...
  END AS 'DivisionCode',                                       
  MU.EmployeeNo 'EmployeeCode',                                      
  HBM.CurrencyCode as 'SalesCurrencyCode',HBM.CurrencyCode as 'PurchCurrencyCode',DisplayDiscountRate 'BaseAmount',0 'TaxAmount',MarkupAmount 'MarkupOnBaseFare',                                      
 ...
 left join Hotel_Room_master HRM on HRM.book_fk_id=HBM.pkId                                      
  ...
  LEFT JOIN agentLogin AL ON  AL.UserID = HBM.RiyaAgentID                                      
  Where         
HBM.BookingReference in ('TNHAPI00075512','TNHAPI00075527','TNHAPI00076410')              
  and        
  Sh.IsActive=1                                       
  and SH.FkStatusId=7                                   
 ...
 select FKHotelBookingId,max(id) as max_id,FkStatusId from Hotel_Status_History as cc                                            
   ...
   */    
    HSHVouchred.FkStatusId=4                                       
 and ((AL.userTypeID = 2 and B2BR.Icast in (select HotelIcust from tblInterBranchWinyatra)) or AL.userTypeID IN(4))                                      
    AND DATEDIFF(MINUTE,CAST( HB.inserteddate AS datetime),GETDATE())>5    
   AND CAST( HB.inserteddate  AS DATE )= CAST(GETDATE() AS DATE)                                  
   --and HB.BookingReference in ('TNH00267356')--('TNH00267009','TNH00267010')    --TNH00264660                                  
 ...
 'VEND000024'                                      
 when HBM.providerid in ('rt-vtactivities-live') then 'VEND000024'                                      
 when HBM.providerid = 'rt-rtactivities-live ' then 'VEND000012'                                      
 end AS 'PayToVendor'                                      
...
THEN 'CC1013000'   ELSE 'CC1001000' END AS DivisionCode                                 
 ,MU.EmployeeNo 'EmployeeCode'                                      
...
WHERE HSHVouchred.FkStatusId = 4                                      
 ...
 --end                                      
  -- and HBM.BookingRefId in ('TNA0001247')                                      
