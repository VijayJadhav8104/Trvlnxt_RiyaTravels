...
--Modified Date :    
-- Exec RiyaCancelHrs_Test '33435','hotelbeds','72.53333','05/26/2022'  
--RiyaCancelHrs_Test '33435','agoda','90.5','05/25/2022'
--===================================================    
CREATE PROCEDURE RiyaCancelHrs_Test   
 ...
 --===========================================================    
    ...
    WHERE ltrim(rtrim(HSM.SupplierName)) = ltrim(rtrim(@SupplierName))  --ltrim(rtrim('Qtech (LocalSystem)'))    
     ...
     AND HSM.IsActive = 1    
        ...
        --set @CanDeadDate=Convert(varchar,dateadd(day,-0,@CanDeadDate),100) 
        
        set @CanDays=(@CancelHr+ISNULL(@CanHr,0))/24
        
 Select Convert(datetime2 ,@CanDeadDate , 14) as 'CancenllationDeadLine', 
 FORMAT(CAST(@CanDeadDate AS datetime) , 'dd-MM-yyyy HH:mm tt') as 'CancenllationDeadLine_new',
    ...
    -- ,convert(VARCHAR, dateadd(hour, - 0, @CanDeadDate), 105) AS 'CDate'  
        ,@CanDeadDate as 'CDate'-- @CancelHr,@CanHr
		, @ChInDate as ChInDate
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[RiyaCancelHrs_Test] TO [rt_read]
    AS [dbo];
