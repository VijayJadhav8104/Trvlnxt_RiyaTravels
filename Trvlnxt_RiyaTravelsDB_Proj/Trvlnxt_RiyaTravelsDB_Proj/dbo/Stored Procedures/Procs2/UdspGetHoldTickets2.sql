...
CREATE PROC [dbo].[UdspGetHoldTickets2]   
...
BEGIN  
 ...
 BEGIN  
  ...
  , t.mobileNo AS mob  
   ...
   ,case when t.VendorName IN ('SQNDC','GFNDC','EKNDC','WYNDC','EYNDC','AmadeusNDC','Sabre','Verteil','TKNDC', 'VerteilNDC','Indigo','Spicejet','ETNDC','FlyDubai') then 'APIOUT' else t.VendorName end as VendorName  
   , t.VendorName  
   , t.FareSellKey  
 ...
 BEGIN  
  ...
  , GDSPNR AS GDSPNR  
   ,CASE  WHEN airlinePNR IS NULL THEN @AirlinePNR   ELSE airlinePNR END AS airlinePNR  
    ...
    , t.Country  
    , t.OfficeID  
    ,case when t.VendorName IN ('SQNDC','GFNDC','EKNDC','WYNDC','Sabre','EYNDC','AmadeusNDC','Verteil','TKNDC', 'VerteilNDC','Indigo','Spicejet','ETNDC','FlyDubai') then 'APIOUT' else t.VendorName end as VendorName  
    , t.AgentID  
    , t.IsBooked  
    , t.AgentID  
    , br.AgencyName    
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[UdspGetHoldTickets2] TO [rt_read]
    AS [dbo];
