...
-- =============================================  
--exec [TrvlNxt_SearchprocessdataAgentByStatusNEW1] '2023-05-08','2023-05-08', NULL, 'D1K2X6', NULL, NULL, NULL, '99', '99', NULL, 0, 0, 3, '', 3, 0, NULL, 0,5000, NULL
CREATE PROCEDURE [dbo].[TrvlNxt_SearchprocessdataAgentByStatusNEW1]  
 @FROMDate Date = NULL  
 , @ToDate Date = NULL  
...
BEGIN  
 ...
 SET @NewsUserID = (CASE WHEN @Userid = 0 THEN '' ELSE CAST(@Userid AS VARCHAR(10)) END)  
   ...
   Set @GdsPNR = (Select top 1 gdspnr from tblbookmaster where riyaPNR=@PNR or GDSPNR=@PNR)
 ...
 BEGIN  
    ...
               , a.RiyaPNR  
     ...
     , (SELECT STUFF((SELECT '/' + s.paxFName + ' ' + paxLName   
      ...
      FROM (SELECT * FROM (SELECT pid, paxFName, paxLName, tblPassengerBookDetails.fkBookMaster, ROW_NUMBER() OVER(PARTITION BY tblPassengerBookDetails.fkBookMaster ORDER BY pid) AS RN  
			...
			LEFT OUTER JOIN tblBookItenary ON tblBookMaster.pkId = tblBookItenary.fkBookMaster
			WHERE (@PNR IS NULL OR @PNR = '' OR (tblBookMaster.riyaPNR = @PNR OR tblBookMaster.GDSPNR = @PNR OR tblBookItenary.airlinePNR = @PNR))
			)
  ...
  BEGIN   
   ...
   (SELECT pid, ISNULL(paxfname, '')+ ' ' + paxlname AS paxname  
     ...
     , (SELECT STUFF((SELECT '/' + s.toSector FROM tblBookItenary s WHERE s.orderId = t.orderId FOR XML PATH('')), 1, 1, '') AS Sector FROM tblBookItenary t WHERE t.fkBookMaster = a.pkId  GROUP BY t.orderId) AS 'tosector'  
     , (SELECT STUFF((SELECT '/' + s.frmSector + '-' + toSector FROM tblBookItenary s WHERE s.orderId = t.orderId FOR XML PATH('')), 1, 1, '') AS Sector FROM tblBookItenary t WHERE t.fkBookMaster = a.pkId  GROUP BY t.orderId) AS 'Sector'  
     , (SELECT STUFF((SELECT '-' +(SELECT CONVERT(VARCHAR(10), CONVERT(DATETIME, s.depDate, 0), 103 ))  FROM tblBookItenary s WHERE s.orderId = t.orderId FOR XML PATH('')), 1, 1, '') AS Sector FROM tblBookItenary t WHERE t.fkBookMaster = a.pkId  GROUP BY 
t.orderId) AS 'Dep Date all'  
	 ...
	 from (SELECT * from 
				(SELECT pid, paxFName, paxLName, tblPassengerBookDetails.fkBookMaster, ROW_NUMBER() OVER(Partition by tblPassengerBookDetails.fkBookMaster ORDER BY pid) AS RN   
				from tblPassengerBookDetails 
				LEFT OUTER JOIN tblBookMaster ON tblPassengerBookDetails.fkBookMaster = tblBookMaster.pkId
				LEFT JOIN tblBookItenary ON tblBookMaster.pkId = tblBookItenary.fkBookMaster
				WHERE (@PNR IS NULL OR @PNR = '' OR (tblBookMaster.riyaPNR = @PNR OR tblBookMaster.GDSPNR = @PNR OR tblBookItenary.airlinePNR = @PNR))
 ...
 BEGIN  
  ...
  BEGIN   
   ...
   SELECT * INTO #tempTableB55 FROM  
    ...
    , (SELECT STUFF((SELECT '/' + s.toSector FROM tblBookItenary s WHERE s.orderId = t.orderId FOR XML PATH('')), 1, 1, '') AS Sector FROM tblBookItenary t WHERE t.fkBookMaster = a.pkId  GROUP BY t.orderId) AS 'tosector'  
    , (SELECT STUFF((SELECT '/' + s.frmSector + '-' + toSector FROM tblBookItenary s WHERE s.orderId = t.orderId FOR XML PATH('')), 1, 1, '') AS Sector FROM tblBookItenary t WHERE t.fkBookMaster = a.pkId  GROUP BY t.orderId) AS 'Sector'  
    , (SELECT STUFF((SELECT '-' +(SELECT CONVERT(VARCHAR(10), CONVERT(DATETIME, s.depDate, 0), 103))  FROM tblBookItenary s WHERE s.orderId = t.orderId FOR XML PATH('')), 1, 1, '') AS Sector FROM tblBookItenary t WHERE t.fkBookMaster = a.pkId  GROUP BY t.
orderId) AS 'Dep Date all'  
    , (SELECT STUFF((SELECT '/' + s.paxFName + ' ' + paxLName FROM tblPassengerBookDetails s WHERE s.fkBookMaster = t.fkBookMaster FOR XML PATH('')), 1, 1, '') AS AllPax FROM tblPassengerBookDetails t WHERE t.fkBookMaster = a.pkId  GROUP BY t.fkBookMaster
) AS 'AllPax'   
       FROM (SELECT * from (SELECT pid, paxFName, paxLName, tblPassengerBookDetails.fkBookMaster, ROW_NUMBER() OVER(Partition by tblPassengerBookDetails.fkBookMaster ORDER BY pid) AS RN 
	   from tblPassengerBookDetails
	   LEFT OUTER JOIN tblBookMaster ON tblPassengerBookDetails.fkBookMaster = tblBookMaster.pkId
	   LEFT JOIN tblBookItenary ON tblBookMaster.pkId = tblBookItenary.fkBookMaster
	   where CheckboxVoid = 1 and (@PNR IS NULL OR @PNR = '' OR (tblBookMaster.riyaPNR = @PNR OR tblBookMaster.GDSPNR = @PNR OR tblBookItenary.airlinePNR = @PNR))
	   ) AS tblPBD where rn = 1 ) AS T1   
       JOIN tblBookMaster  a ON a.pkId = t1.fkBookMaster     
       LEFT JOIN tblBookItenary tbi ON a.pkId = tbi.fkBookMaster   
       and a.frmSector = tbi.frmSector     
