...
CREATE procedure [dbo].[sp_MISReport]
	@FromDate Datetime,
	@Todate Datetime,
	@AgentID Varchar(50),
...
AS
BEGIN
	SELECT inserteddate_old AS 'Issue Date'
			...
			, STUFF((SELECT ',' + convert(varchar(11),s.depDate,103) FROM tblBookItenary s WITH(NOLOCK) WHERE s.orderId = B.orderId FOR XML PATH('')),1,1,'') AS 'Date of Travel'
			, STUFF((SELECT ',' + convert(varchar(11),s.arrivalDate,103) FROM tblBookItenary s WITH(NOLOCK) WHERE s.orderId = B.orderId FOR XML PATH('')),1,1,'') AS [Travel End Date]
	...
	LEFT JOIN mAttrributesDetails A WITH(NOLOCK) ON A.OrderID=B.orderId
	...
	AND ((@AgencyNameList = '' or @AgencyNameList = '-1') OR (b.AgentID IN ( SELECT Data FROM sample_split(@AgencyNameList,','))))
	AND P.totalFare > 0
	AND B.totalFare > 0
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[sp_MISReport] TO [rt_read]
    AS [dbo];
