...
--sp_helptext Sp_GetAgentAccountStatementProductWise_755 '2025-01-01','2025-02-26','','','4','33435','IN','Transfer',''                                        
CREATE PROCEDURE [dbo].[Sp_GetAgentAccountStatementProductWise_755]                                        
 @FROMDate Date=NULL                                         
 , @ToDate Date=NULL                                        
...
BEGIN                                        
 ...
 SELECT * INTO #tempTableAASAir FROM (                                        
  ...
  WHEN B.BookingStatus = 6 AND  coun.CountryCode = 'SA' THEN (DATEADD(SECOND, -1 * 60 * 60 - 29 * 60 - 13,CONVERT(varchar(20),b.BookingTrackModifiedOn,120)))                         
 WHEN B.BookingStatus = 6 AND  coun.CountryCode = 'US' THEN (DATEADD(SECOND, - 9 * 60 * 60 - 29 * 60 - 16,CONVERT(varchar(20),b.BookingTrackModifiedOn,120))) -- 9 hour, 29 minutes AND 16 seconds                                        
  ...
  WHEN B.BookingStatus != 6 AND  coun.CountryCode = 'SA' THEN (DATEADD(SECOND, -1 * 60 * 60 - 29 * 60 - 13,CONVERT(varchar(20),ISNULL(b.IssueDate,ISNULL(b.inserteddate_old,b.inserteddate)),120))) -- 1 hour, 29 minutes AND 13 seconds                       
...
   AND (IsBooked=1 OR BookingStatus=1)                                        
  ...
  , case when coun.CountryCode='AE' then (DATEADD(SECOND, -1*60*60 -29*60 -13,CONVERT(varchar(20),ab.createdon,120)))-- 1 hour, 29 minutes and 13 seconds      
   when coun.CountryCode='SA' then (DATEADD(SECOND, -1*60*60 -29*60 -13,CONVERT(varchar(20),ab.createdon,120)))    
...
   AND ((@AgentId='') or ( AB.AgentNo = cast(@AgentId as varchar(50))))                                 
  ...
  , CASE WHEN coun.CountryCode='AE' THEN (DATEADD(SECOND, -1*60*60 -29*60 -13,CONVERT(varchar(20),ISNULL(ab.createdon,(CASE WHEN B.BookingStatus=6 THEN b.BookingTrackModifiedOn ELSE ISNULL(ab.CreatedOn,b.inserteddate)END)),120)))     
   WHEN coun.CountryCode='SA' THEN (DATEADD(SECOND, -1*60*60 -29*60 -13,CONVERT(varchar(20),ISNULL(ab.createdon,(CASE WHEN B.BookingStatus=6 THEN b.BookingTrackModifiedOn ELSE ISNULL(ab.CreatedOn,b.inserteddate)END)),120)))                         
...
WHEN coun.CountryCode='SA' THEN (DATEADD(SECOND, -1*60*60 -29*60 -13,CONVERT(varchar(20),ISNULL(ab.createdon,(CASE WHEN B.BookingStatus=6 THEN b.BookingTrackModifiedOn ELSE ISNULL(b.IssueDate,ISNULL(b.inserteddate_old,b.inserteddate)) END)),120)     
)    
)    
...
   AND BookingStatus !=2 AND BookingStatus !=5 AND BookingStatus !=0 AND (b.totalFare>0 OR (ISNULL(b.PreviousRiyaPNR,'')!='')) AND (P.payment_mode='Check' OR P.payment_mode='Credit')                                        
  ...
  , CASE WHEN coun.CountryCode='AE' THEN (DATEADD(SECOND, -1*60*60 -29*60 -13,CONVERT(varchar(20),ISNULL(ab.createdon,(CASE WHEN B.BookingStatus=6 THEN b.BookingTrackModifiedOn else ISNULL(b.inserteddate_old,b.inserteddate) END)),120)))      
   ...
   WHEN coun.CountryCode='SA' THEN (DATEADD(SECOND, -1*60*60 -29*60 -13,CONVERT(varchar(20),ISNULL(ab.createdon,(CASE WHEN B.BookingStatus=6 THEN b.BookingTrackModifiedOn else ISNULL(b.inserteddate_old,b.inserteddate) END)),120)))                       
...
, (B.HotelName + ' ' + B.HotelAddress1) AS 'Description'                                      
  ...
  AND ((@FROMDate = '' or @ToDate='') or (cast(B.inserteddate as date) between @FROMDate and @ToDate ))                             
   ...
   SELECT                                       CAST(B.pkId AS VARCHAR(50)) AS BookMasterID                                    
   , BR.Icast as 'AgencyID'                                    
   , BR.AgencyName AS 'AgencyName'                                      
   ,B.inserteddate  AS 'DateTime'                                  
   , (B.HotelName + ' ' + B.HotelAddress1) AS 'Description'                                    
...
AS 'Travel date'          
 ...
 --,(TRC.CarName + ' ' + TRC.PricingPackageType) AS 'Description'                                      
   , B.BookingRefId AS 'Booking id'                                      
