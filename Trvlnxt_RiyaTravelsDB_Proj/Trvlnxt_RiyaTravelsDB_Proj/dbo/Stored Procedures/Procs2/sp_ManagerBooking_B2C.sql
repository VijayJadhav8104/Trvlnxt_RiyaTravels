CREATE proc [dbo].[sp_ManagerBooking_B2C]
...
with cte as
			(SELECT a.pkId , a.airName as owairName,a.frmSector as owfrmSector,a.toSector as owtoSector,a.deptTime as owdepDate,a.arrivalDate as owarrivalDate,a.fromAirport as owfromAirport,
				a.deptTime as owdeptTime,a.fromTerminal as owfromTerminal,a.toTerminal as owtoTerminal,(SELECT TOP 1 airlinePNR FROM tblBookItenary BI WITH(NOLOCK)  WHERE BI.fkBookMaster=a.pkId) as owGDSPNR,a.arrivalTime as owarrivalTime,a.TotalMarkup as confee,
				...
				b.TotalTime as rtTotalTime,(SELECT TOP 1 airlinePNR FROM tblBookItenary BI WITH(NOLOCK)  WHERE BI.fkBookMaster=b.pkId) as rtGDSPNR,b.airCode as rtCarrier,b.fromAirport as rtFromAirport,b.toAirport as rtoAirport,
			...
			case when a.canceledDate is null then 0 else 1 end as CancelFlag
				...
				,(select top 1 airlinePNR from tblBookItenary bItnry WITH(NOLOCK)  where a.pkid=bItnry.fkBookMaster AND a.operatingCarrier=bItnry.operatingCarrier) as AirlinePNR,a.toAirport,a.orderId,
				...
				,(SELECT TOP 1(IsRefunded) FROM tblPassengerBookDetails pax  WITH(NOLOCK) WHERE a.pkid=pax.fkBookMaster) as RefundFlag,a.Country
			...
			FROM tblBookMaster a WITH(NOLOCK) 
				...
				left join Paymentmaster p  WITH(NOLOCK) on p.order_id=a.orderId
