...
--============================================                                  
CREATE PROC [Hotel].GetManualHotelBookingDetailsByID                           
...
BEGIN                               
   ...
   IN (SELECT Service FROM temp) FOR XML PATH('')), 1, 1, '' ) AS 'Pax' ,providerConfirmationNumber,cancellationToken             
   ...
   SELECT hrm.RFinalPrice,hrm.RbaseRate,hrm.RpublishedRate,hrm.RpublishedRate AS RtotalRate,  hrm.Room_Id,                      
   CASE hm.IsGSTRequired WHEN 1 THEN 'true' ELSE 'false' END AS IsGSTRequired,hm.CurrencyCode,                      
   hm.SupplierName,hm.SupplierUsername,CASE hm.Refundable WHEN 1 Then 'true' WHEN 0 THEN 'false' END AS Refundable,                     
   CASE hm.PayAtHotel WHEN 1 Then 'true' WHEN 0 THEN 'false' END AS PayAtHotel,                      
   CASE hm.IsPANCardRequired WHEN 1 THEN 'true' WHEN 0 THEN 'false' END AS IsPANCardRequired,                
   CASE hm.Refundable WHEN 1 THEN 'Refundable' WHEN 0 THEN 'NonRefundable' END AS refundability,hm.Meal, hm.HotelIncludes,            
   hrm.RateId AS RateId,hrm.room_class_id,hrm.numOfAdults,hrm.numOfChildren,hc.RiyaCommission,hm.SCommissionDiscription,      
   rrate.Amount, rrate.taxIncluded,CONVERT(DATETIMEOFFSET, rrate.Date, 127) AS dDate,rrate.discount,                
   CASE rrate.taxIncluded WHEN 1 THEN 'true' WHEN 0 THEN 'false' END AS taxIncluded         
   FROM Hotel_Room_master hrm LEFT JOIN hotel_bookmaster hm ON hm.pkId=hrm.book_fk_id       
   LEFT JOIN B2BHotel_Commission hc ON hm.pkId=hc.Fk_BookId         
   LEFT JOIN [Hotel].[Hotel_RoomRatesPerNight] rrate ON rrate.Roomfkid=hrm.Room_Id          
   WHERE pkid=@PkId       
...
--Exec [Hotel].GetManualHotelBookingDetailsByID_2 'TNHAPI00001501'                                  
