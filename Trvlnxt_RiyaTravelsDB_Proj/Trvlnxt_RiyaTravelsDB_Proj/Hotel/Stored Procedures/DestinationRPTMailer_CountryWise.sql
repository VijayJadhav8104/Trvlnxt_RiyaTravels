--[Hotel].[DestinationRPTMailer_CountryWise] 'united arab emirates'   
--[Hotel].[DestinationRPTMailer_CountryWise] 'UNITED STATE OF AMERICA'   
  

  --exec [Hotel].[DestinationRPTMailer_CountryWise] 'Canada'
  
CREATE PROC [Hotel].[DestinationRPTMailer_CountryWise]        
    
 @selectCountry varchar(200)=''    
AS      
...
BEGIN      
 ...
 Left join Hotel_Status_History SH on HB.pkId=SH.FKHotelBookingId      
    WHERE FkStatusId IN (3,4)       
            ...
            AND (      
                --UPPER(ISNULL(HotelBookCountryName, '')) IN (SELECT CountryName FROM @Countries)      
              -- OR 
			  UPPER(ISNULL(CountryName, '')) IN (SELECT CountryName FROM @Countries)      
            )      
        GROUP BY       
    ...
    SELECT       
        ...
        SupplierName,      
        Booking_Count,      
        isnull(Amount_in_INR,0) as Amount_in_INR,      
         isnull(Total_Room_Nights,0) as Total_Room_Nights,      
        SUM(isnull(Booking_Count,0)) OVER (PARTITION BY Country) AS Total_Booking_Count_By_Country,      
        SUM(isnull(Amount_in_INR,0)) OVER (PARTITION BY Country) AS Total_Amount_in_INR_By_Country,      
        SUM(isnull(Total_Room_Nights,0)) OVER (PARTITION BY Country) AS Total_Room_Nights_By_Country,      
        SortOrder      
    FROM (      
