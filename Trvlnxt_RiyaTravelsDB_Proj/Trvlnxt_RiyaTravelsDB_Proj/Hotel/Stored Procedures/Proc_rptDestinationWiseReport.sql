...
--EXEC [Hotel].[Proc_rptDestinationWiseReport] '01-Mar-2025', '07-Mar-2025'  
--select distinct * from B2BHotelSupplierMaster  
  
CREATE PROCEDURE [Hotel].[Proc_rptDestinationWiseReport]  
...
BEGIN  
  ...
  SET @sql = N'SELECT upper(Ltrim(Rtrim(HB.cityName))) as cityName,   
...
END  
    ...
    SET @sql = @sql + N' GROUP BY HB.cityName, HotelBookCountryName,countryname, convert(date, HB.inserteddate)';  
