...
-- =============================================              
CREATE PROCEDURE [Hotel].[insertApiBookData]              
...
@TotalExtraServiceCharges varchar(100)=null
AS              
BEGIN              
...
if(@AmOfficeId IS NOT NULL and @AmOfficeId!='')          
 ...
 if(@TotalExtraServiceCharges IS NOT NULL and @AmRateCode!='')          
 BEGIN          
   insert into Hotel.ApiBookData(BookingId,[Key],[Value],ClientBookingID) values(@bookingId,'TotalServieCharges',@TotalExtraServiceCharges,@ClientBookingId);            
 END   
