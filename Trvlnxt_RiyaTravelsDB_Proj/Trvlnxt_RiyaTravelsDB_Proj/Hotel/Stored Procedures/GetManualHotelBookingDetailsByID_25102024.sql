...
--============================================                        
CREATE PROC [Hotel].GetManualHotelBookingDetailsByID_25102024                
...
BEGIN                     
   ...
   STUFF ((SELECT ',' + t2.Salutation + ' ' + t2.FirstName + ' ' + t2.LastName 
   FROM Hotel_Pax_master AS t2 WHERE t2.room_fk_id IN (SELECT Service FROM temp) FOR XML PATH('')), 1, 1, '' ) AS 'Pax' ,providerConfirmationNumber,
   cancellationToken,DestinationCountryCode,Lat,lang,CurrencyCode,SupplierCurrencyCode,ROEValue,SupplierINRROEValue,
   case nationalty when 'IN' then 0 else 1 end as IsPassportRequired
   ...
   hpm.Pancard,hpm.PassportNum,hpm.Contact,hpm.room_fk_id,IssueDate,Expirydate,PassPortDOB,hpm.PanCardName,hpm.IsLeadPax                         
   ...
   SELECT Amount, taxIncluded,CONVERT(DATETIMEOFFSET, [Date], 127) AS dDate,discount, CASE taxIncluded WHEN 1 THEN 'true' WHEN 0 THEN 'false' END AS taxIncluded              
   FROM [Hotel].[Hotel_RoomRatesPerNight] WHERE FkBookingId=@PkId    
   
   --Rates Booking Policy
   SELECT PolicyType,PolicyText FROM Hotel.HotelCancelBookPolicies WHERE FKBookingId=@PkId AND GroupName='Booking Policies'

   SELECT Type,Description,Frequency,Unit,Amount,Currency,Text FROM Hotel.AdditionalCharges WHERE FkBookId=@PkId 
                                   
End                
                
--Exec [Hotel].GetManualHotelBookingDetailsByID_25102024 'TNHAPI00004483' -- '174214','174215'                      
--select * from Hotel_BookMaster where BookingReference ='TNHAPI00001488'-- like '%TNHAPI%'               
--select * from Hotel_BookMaster where BookingReference like '%TNHAPI%'and TotalRooms>1  ORDER BY inserteddate desc              
--select * from Hotel_Room_master where book_fk_id=64011              
--select * from Hotel_Pax_master where book_fk_id=64011              
--select * from [Hotel].HotelCancelBookPolicies where FKBookingId=64011