...
CREATE proc [Hotel].[Proc_RetriveHotelReconInfo_1] -- 'TNHAPI00009776'                                  
...
Begin                                      
       	;with dt as (
select distinct BookID,Count(status) 'Count',Status,RowFlag
--,isnull(Check_in_date,'01-01-1753' )
from hotel.HotelReconRpt
 group by
 BookID,Status,RowFlag,Check_in_date
 having
 --BookID='TNH00253778' 
(Status='Failed'and RowFlag in  ('Dynamo' , 'Supplier' ) ) and Count(Status)>3 
)                                   
...
, case when isnull(PCC,'')='' then  null else PCC end  as officeId          
  ...
  where                           
             ...
             cast(CheckOutDate as Date)>= cast(getdate()-2 as date)  
			 ANd cast(sh.CreateDate as date)>=cast(GETDATE()-1 as date)       
         and sh.IsActive=1                       
                  and BookingReference not in(select BookID from dt )
