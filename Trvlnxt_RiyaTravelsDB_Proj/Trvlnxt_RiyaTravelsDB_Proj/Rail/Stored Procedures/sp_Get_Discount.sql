 ...
 CREATE PROCEDURE [Rail].[sp_Get_Discount]            
 @Id varchar(20)=Null        
 AS         
 BEGIN        
 ...
 Case when AgentId!='All' then ( select    
    distinct      
     stuff((    
         select ',' + u.AgentId    
         from [Rail].[Discount_AgentMapping] u    
  ...
  from [Rail].[Discount_AgentMapping]) else 'All' End as AgencyName    
  FROM [Rail].[tbl_Discount] S    
