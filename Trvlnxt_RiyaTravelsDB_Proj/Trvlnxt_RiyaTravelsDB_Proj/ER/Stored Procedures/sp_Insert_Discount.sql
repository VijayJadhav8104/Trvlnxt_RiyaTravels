 ...
 CREATE PROCEDURE [ER].[sp_Insert_Discount]                  
 @Id int=null,                
 ...
 Begin                
  ...
  Select top 1 @ServiceID=Id from [ER].[tbl_Discount] order by Id desc          
  DECLARE @Agentstr VARCHAR(300)           
          
 SET @Agentstr =(Select AgentId from [ER].[tbl_Discount] where Id=@ServiceID)           
 If (@Agentstr!='All')          
  BEGIN          
   DECLARE @SEPARATOR CHAR(1)           
   SET @SEPARATOR=','           
             
   INSERT INTO [ER].[Discount_AgentMapping]          
               (agentid,ServiceId)           
...
GRANT VIEW DEFINITION
    ON OBJECT::[ER].[sp_Insert_Discount] TO [rt_read]
    AS [RiyaTravels];
