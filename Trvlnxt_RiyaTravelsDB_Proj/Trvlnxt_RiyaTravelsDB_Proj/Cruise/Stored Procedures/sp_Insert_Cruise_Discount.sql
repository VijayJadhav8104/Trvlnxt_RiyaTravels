 ...
 CREATE PROCEDURE [Cruise].[sp_Insert_Cruise_Discount]                
 @Id int=null,              
 ...
 If(@Id=0)              
 Begin              
  Insert Into [Cruise].tbl_Cruise_Discount                     
  (                    
  MarketPoint,                    
  UserType,                    
  TravelValidityFrom,                    
  ...
  Select top 1 @ServiceID=Id from [Cruise].tbl_Cruise_Discount order by Id desc        
  DECLARE @Agentstr VARCHAR(300)         
        
 SET @Agentstr =(Select AgentId from [Cruise].tbl_Cruise_Discount where Id=@ServiceID)         
 If (@Agentstr!='All')        
  BEGIN        
   DECLARE @SEPARATOR CHAR(1)         
   SET @SEPARATOR=','         
           
   INSERT INTO [Cruise].[Cruise_Discount_AgentMapping]         
               (agentid,ServiceId)         
 ...
 If(@Id>0)              
    Begin              
       Update [Cruise].tbl_Cruise_Discount Set              
...
GRANT VIEW DEFINITION
    ON OBJECT::[Cruise].[sp_Insert_Cruise_Discount] TO [rt_read]
    AS [DB_TEST];
